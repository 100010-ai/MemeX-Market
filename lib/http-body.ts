/**
 * Copy a bounded byte buffer into an ArrayBuffer accepted by Response/Request.
 */
...
/**
 * Reads an incoming Request without ever buffering more than maxBytes.
 * Returns null when the body is empty or exceeds the configured bound.
 */
export async function readRequestBytesLimited(request: Request, maxBytes: number): Promise<Uint8Array | null> {
  const limit = Math.max(1, Math.floor(maxBytes));
  const declared = Number(request.headers.get("content-length") || 0);
  if (Number.isFinite(declared) && declared > limit) {
    await request.body?.cancel("request body exceeds configured limit").catch(() => undefined);
    return null;
  }
