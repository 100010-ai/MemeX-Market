...
-- MXM v0.4.1 upgrade migration.
-- This upgrades legacy v0.2 databases to the strict Telegram-only Gift schema.
-- The migration is transactional: if any statement fails, none of the changes commit.

-- ---------------------------------------------------------------------------
-- 1. Remove legacy seeded/demo market rows.
-- ---------------------------------------------------------------------------

-- v0.2 demo Gifts had no Telegram identity/media. Delete by data provenance,
-- not by collection name: names such as Light Sword/Stellar Rocket are real
-- Telegram collections and must never be removed just because the name matches.
...
-- v0.2 seeded synthetic collection candles. Rebuild the candle table entirely
-- from actual persisted Gift trades so no seeded chart points survive.
delete from public.gift_collection_candles;

insert into public.gift_collection_candles(
  base_name,bucket_start,open,high,low,close,volume
)
select
  ga.base_name,
  date_trunc('hour', gt.created_at) as bucket_start,
  (array_agg(gt.price order by gt.created_at asc))[1] as open,
  max(gt.price) as high,
  min(gt.price) as low,
  (array_agg(gt.price order by gt.created_at desc))[1] as close,
  sum(gt.price) as volume
from public.gift_trades gt
join public.gift_assets ga on ga.id=gt.asset_id
group by ga.base_name, date_trunc('hour', gt.created_at);

delete from public.coins
where creator_profile_id is null
  and symbol in ('KCAT','ERR404','CAPY','BONKO','FROG','PRAT');

-- Legacy system-owned Gift rows are not valid in the Telegram-only market.
delete from public.virtual_gifts
where source_owner_profile_id is null
   or owner_profile_id is null;

-- ---------------------------------------------------------------------------
-- 2. Bring legacy tables up to the current schema.
-- ---------------------------------------------------------------------------

alter table public.trades
  add column if not exists realized_pnl numeric(24,8) not null default 0;

alter table public.gift_trades
  add column if not exists realized_pnl numeric(24,8) not null default 0;

create index if not exists gift_trades_seller_created_idx
  on public.gift_trades(seller_profile_id, created_at desc);

-- Views from v0.2 directly reference demo_emoji/reference_price.
-- leaderboard from v0.2 also references reference_price directly.
-- Drop the dependent views explicitly before removing those legacy columns.
...
alter table public.gift_assets
  ...
  alter column model_file_id set not null,
  alter column symbol_file_id set not null;

-- v0.2 used defaults that allowed placeholder-looking Telegram metadata.
alter table public.gift_assets
  alter column model_rarity_per_mille drop default,
  alter column symbol_rarity_per_mille drop default,
  alter column backdrop_rarity_per_mille drop default,
  alter column backdrop_center_color drop default,
  alter column backdrop_edge_color drop default,
  alter column backdrop_symbol_color drop default,
  alter column backdrop_text_color drop default;

do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conrelid='public.gift_assets'::regclass
      and conname='gift_assets_backdrop_center_color_v041_check'
  ) then
    alter table public.gift_assets
      add constraint gift_assets_backdrop_center_color_v041_check
      check (backdrop_center_color between 0 and 16777215);
  end if;

  if not exists (
    select 1 from pg_constraint
    where conrelid='public.gift_assets'::regclass
      and conname='gift_assets_backdrop_edge_color_v041_check'
  ) then
    alter table public.gift_assets
      add constraint gift_assets_backdrop_edge_color_v041_check
      check (backdrop_edge_color between 0 and 16777215);
  end if;

  if not exists (
    select 1 from pg_constraint
    where conrelid='public.gift_assets'::regclass
      and conname='gift_assets_backdrop_symbol_color_v041_check'
  ) then
    alter table public.gift_assets
      add constraint gift_assets_backdrop_symbol_color_v041_check
      check (backdrop_symbol_color between 0 and 16777215);
  end if;

  if not exists (
    select 1 from pg_constraint
    where conrelid='public.gift_assets'::regclass
      and conname='gift_assets_backdrop_text_color_v041_check'
  ) then
    alter table public.gift_assets
      add constraint gift_assets_backdrop_text_color_v041_check
      check (backdrop_text_color between 0 and 16777215);
  end if;
end $$;

alter table public.gift_assets drop column if exists demo_emoji;
alter table public.gift_assets drop column if exists reference_price;
alter table public.gift_assets drop column if exists source;

drop index if exists public.gift_assets_traits_idx;
create index gift_assets_traits_idx
  on public.gift_assets(base_name, model_name, backdrop_name, symbol_name);

create unique index if not exists gift_assets_base_number_unique_idx
  on public.gift_assets(base_name, gift_number);
...
alter table public.virtual_gifts
  alter column source_owner_profile_id set not null,
  alter column owner_profile_id set not null;

alter table public.virtual_gifts
  drop constraint if exists virtual_gifts_source_owner_profile_id_fkey,
  drop constraint if exists virtual_gifts_owner_profile_id_fkey;

alter table public.virtual_gifts
  add constraint virtual_gifts_source_owner_profile_id_fkey
    foreign key (source_owner_profile_id)
    references public.profiles(id)
    on delete restrict,
  add constraint virtual_gifts_owner_profile_id_fkey
...
begin
  if not exists (
    ...
    alter table public.virtual_gifts
      ...
      check (
        (status = 'listed' and listing_price is not null)
        or
        (status = 'owned' and listing_price is null)
      );
  end if;
end $$;
...
create table if not exists public.market_events (
  ...
  check (
    ...
    or
    (kind='listing' and virtual_gift_id is not null and coin_id is null)
  )
);
...
-- ---------------------------------------------------------------------------
-- 3. Re-apply current realtime policies/publication membership.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. Replace RPCs with the current MXM implementations.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 5. Recreate current views, missions and function ACLs.
-- ---------------------------------------------------------------------------
