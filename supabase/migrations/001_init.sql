...
create table if not exists public.gift_assets (
  id uuid primary key default gen_random_uuid(),
  source text not null default 'telegram' check (source in ('telegram','demo')),
  telegram_name text unique,
  gift_id text,
  base_name text not null,
  gift_number integer not null check (gift_number > 0),
  model_name text not null,
  model_rarity_per_mille integer not null default 0 check (model_rarity_per_mille >= 0),
  model_rarity text,
  model_file_id text,
  ...
  symbol_rarity_per_mille integer not null default 0 check (symbol_rarity_per_mille >= 0),
  symbol_file_id text,
  symbol_thumb_file_id text,
  backdrop_name text not null,
  backdrop_rarity_per_mille integer not null default 0 check (backdrop_rarity_per_mille >= 0),
  backdrop_center_color integer not null default 2105376,
  backdrop_edge_color integer not null default 1052688,
  backdrop_symbol_color integer not null default 3158064,
  backdrop_text_color integer not null default 16777215,
  is_premium boolean not null default false,
  is_from_blockchain boolean not null default false,
  demo_emoji text,
  reference_price numeric(24,8) not null default 50 check (reference_price >= 0),
  discovered_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (source, base_name, gift_number)
);
create index if not exists gift_assets_collection_idx on public.gift_assets(base_name, gift_number);
create index if not exists gift_assets_traits_idx on public.gift_assets(model_name, backdrop_name, symbol_name);
...
create table if not exists public.virtual_gifts (
  ...
  source_owner_profile_id uuid references public.profiles(id) on delete set null,
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  check ((status = 'listed' and listing_price is not null) or status = 'owned')
);
...
create unique index if not exists gift_offers_one_pending_idx
  on public.gift_offers(virtual_gift_id, buyer_profile_id) where status = 'pending';
...
-- The browser never receives a Supabase secret and never mutates tables directly.
-- All writes go through authenticated Next.js routes and service-role-only RPCs.
...
begin
  ...
  values (p_telegram_id, nullif(p_username,''), coalesce(nullif(p_first_name,''),'Trader'), nullif(p_last_name,''), nullif(p_photo_url,''))
...
begin
  ...
  insert into public.trades(profile_id, coin_id, side, quote_amount, token_amount, price)
  values (p_profile_id, p_coin_id, 'buy', p_quote_amount, v_token_out, v_exec_price);
...
declare
  ...
  v_quote_out numeric; v_exec_price numeric; v_cost_reduction numeric;
begin
  ...
  v_cost_reduction := case when v_holding.quantity = 0 then 0 else v_holding.cost_basis * (p_token_amount / v_holding.quantity) end;
  ...
  insert into public.trades(profile_id, coin_id, side, quote_amount, token_amount, price)
  values (p_profile_id, p_coin_id, 'sell', v_quote_out, p_token_amount, v_exec_price);
  perform public.record_candle(p_coin_id, v_coin.current_price, v_quote_out);
  perform public.bump_mission(p_profile_id, 'coin_trade', 1);

  return jsonb_build_object('side','sell','quoteAmount',v_quote_out,'tokenAmount',p_token_amount,'executionPrice',v_exec_price,'newPrice',v_coin.current_price);
end;
$$;
...
returns jsonb language plpgsql security definer set search_path = public as $$
declare v_gift public.virtual_gifts; v_asset public.gift_assets; v_buyer public.profiles; v_price numeric; v_seller uuid;
begin
  select * into v_gift from public.virtual_gifts where id=p_virtual_gift_id for update;
  ...
  if v_seller is not null then perform 1 from public.profiles where id=v_seller for update; end if;
  ...
  if v_seller is not null then update public.profiles set balance=balance+v_price where id=v_seller; end if;
  ...
  insert into public.gift_trades(virtual_gift_id,asset_id,buyer_profile_id,seller_profile_id,price)
  values(p_virtual_gift_id,v_gift.asset_id,p_buyer_id,v_seller,v_price);
  perform public.record_gift_collection_candle(v_asset.base_name,v_price);
  perform public.bump_mission(p_buyer_id,'gift_buy',1);
  if v_seller is not null then perform public.bump_mission(v_seller,'gift_sell',1); end if;
  return jsonb_build_object('price',v_price,'virtualGiftId',p_virtual_gift_id);
end;
$$;
...
begin
  ...
  if not found then raise exception 'Gift not found'; end if;
  if v_gift.owner_profile_id is null then raise exception 'System listings do not accept offers'; end if;
...
returns jsonb language plpgsql security definer set search_path = public as $$
declare v_offer public.gift_offers; v_gift public.virtual_gifts; v_asset public.gift_assets; v_buyer public.profiles;
begin
  ...
  insert into public.gift_trades(virtual_gift_id,asset_id,buyer_profile_id,seller_profile_id,price)
  values(v_gift.id,v_gift.asset_id,v_offer.buyer_profile_id,p_owner_id,v_offer.amount);
  ...
  return jsonb_build_object('status','accepted','price',v_offer.amount);
end;
$$;
...
returns void language plpgsql security definer set search_path = public as $$
begin
  update public.gift_offers set status='cancelled'
...
select
  ...
  coalesce((select sum(ca.volume) from public.candles ca where ca.coin_id=c.id and ca.bucket_start>=now()-interval '24 hours'),0) as volume_24h,
...
select
  ga.id as asset_id,vg.id as virtual_gift_id,ga.source,ga.telegram_name,ga.gift_id,ga.base_name,ga.gift_number,
  ...
  ga.is_premium,ga.is_from_blockchain,ga.demo_emoji,ga.reference_price,
  vg.owner_profile_id,coalesce(nullif(op.username,''),op.first_name) as owner_name,vg.listing_price,vg.last_sale_price,vg.status,vg.created_at
from public.gift_assets ga
join public.virtual_gifts vg on vg.asset_id=ga.id
left join public.profiles op on op.id=vg.owner_profile_id;
...
select
  ...
  coalesce((select sum(coalesce(vg.listing_price,vg.last_sale_price,ga.reference_price)) from public.virtual_gifts vg join public.gift_assets ga on ga.id=vg.asset_id where vg.owner_profile_id=p.id),0) as gift_value,
  p.balance
    ...
    + coalesce((select sum(coalesce(vg.listing_price,vg.last_sale_price,ga.reference_price)) from public.virtual_gifts vg join public.gift_assets ga on ga.id=vg.asset_id where vg.owner_profile_id=p.id),0) as net_worth,
...
insert into public.missions(key,period,title,description,reward,target,action_type,sort_order) values
  ('open_app','onboarding','Welcome to MemeX','Open the Mini App and sync your Telegram profile.',10,1,'open_app',10),
  ('sync_gifts','onboarding','Import Telegram Gifts','Pull your collectible gift metadata into the virtual market.',15,1,'sync_gift',20),
  ('first_coin_trade','onboarding','First candle','Complete your first meme-coin trade.',15,1,'coin_trade',30),
  ('first_gift_buy','onboarding','First collectible','Buy your first virtual Telegram Gift.',20,1,'gift_buy',40),
  ('daily_trades','daily','Three fills','Complete 3 meme-coin trades today.',12,3,'coin_trade',100),
  ('daily_offer','daily','Make an offer','Place an offer on a player-owned virtual gift.',8,1,'gift_offer',110),
  ('daily_listing','daily','List a gift','List one of your virtual gifts for sale.',8,1,'gift_list',120),
  ('weekly_market','weekly','Market regular','Complete 15 meme-coin trades this week.',45,15,'coin_trade',200),
  ('weekly_collector','weekly','Collector run','Buy 3 virtual gifts this week.',50,3,'gift_buy',210),
  ('weekly_creator','weekly','Launch a meme','Create one meme coin this week.',35,1,'create_coin',220)
...
-- RPCs are not public API. Next.js uses the service-role key after Telegram session validation.
