...
const replacement = `${marker}    thumbnail_url: String(process.env.APP_CANONICAL_URL || process.env.NEXT_PUBLIC_APP_URL || "https://meme-x-market.vercel.app").trim().replace(/\\/$/, "") + "/api/telegram/inline-icon?v=652",\n    thumbnail_width: 96,\n    thumbnail_height: 96,\n`;

source = source.replace(marker, replacement);
fs.writeFileSync(aiPath, source);
console.log("MemeX inline icon patch applied: JPEG thumbnail endpoint added to inline result");
