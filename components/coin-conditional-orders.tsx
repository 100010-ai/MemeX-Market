...
export function CoinConditionalOrders({ coin, holdingQuantity, availableBalance, onBalanceChange }: { coin: Coin; holdingQuantity: number; availableBalance: number; onBalanceChange: () => void }) {
  ...
  return <section className="border-t border-[var(--border-soft)] pt-4">
    <div className="flex items-start justify-between gap-3"><div><p className="flex items-center gap-1.5 text-xs font-medium"><Target size={13}/>Условные заявки</p><p className="mt-1 text-[9px] leading-4 text-[var(--muted)]">Лимитные заявки, фиксация прибыли и ограничение убытка исполняются сервером при достижении цены. Покупка резервирует виртуальные TON.</p></div><span className="text-[9px] text-[var(--muted)]">{active.length} активных</span></div>
    <div className="mt-3 grid grid-cols-2 gap-2"><select value={kind} onChange={(event) => setKind(event.target.value as Kind)} className="mxm-input"><option value="limit_buy">Лимитная покупка</option><option value="limit_sell">Лимитная продажа</option><option value="take_profit">Фиксация прибыли</option><option value="stop_loss">Ограничение убытка</option></select><select value={durationDays} onChange={(event) => setDurationDays(Number(event.target.value))} className="mxm-input"><option value={1}>1 день</option><option value={3}>3 дня</option><option value={7}>7 дней</option><option value={14}>14 дней</option><option value={30}>30 дней</option></select></div>
