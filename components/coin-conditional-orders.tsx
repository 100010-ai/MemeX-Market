...
import { MIN_COIN_BUY_TON, parseEconomyAmount } from "@/lib/economy";
...
export function CoinConditionalOrders({ coin, holdingQuantity, availableBalance, onBalanceChange, compact: compactMode = false }: { coin: Coin; holdingQuantity: number; availableBalance: number; onBalanceChange: () => void; compact?: boolean }) {
  ...
  const parsedInput = parseEconomyAmount(inputAmount);
  const parsedTrigger = parseEconomyAmount(triggerPrice);
  const numericInput = parsedInput ?? 0;
  const numericTrigger = parsedTrigger ?? 0;
  const max = isBuy ? availableBalance : holdingQuantity;
  const triggerDirectionValid = parsedTrigger != null && numericTrigger > 0 && (
    kind === "limit_buy" ? numericTrigger <= coin.currentPrice
      : kind === "stop_loss" ? numericTrigger <= coin.currentPrice
        : numericTrigger >= coin.currentPrice
  );
  const inputValid = parsedInput != null && numericInput > 0 && numericInput <= max && (!isBuy || numericInput >= MIN_COIN_BUY_TON);
  const valid = inputValid && triggerDirectionValid;
  ...
  async function create() {
    ...
    const stableRequestKey = createRequestKey.current || requestKey();
    createRequestKey.current = stableRequestKey;
    try {
      await apiFetch(`/api/coins/${coin.id}/orders`, { method: "POST", body: JSON.stringify({ kind, triggerPrice: numericTrigger, inputAmount: numericInput, durationDays, requestKey: stableRequestKey }) });
      createRequestKey.current = null;
      setInputAmount(""); setTriggerPrice("");
      await load();
      onBalanceChange();
    } catch (cause) {
      // Keep the same request key. If the server committed but the response was
      // lost, retrying the unchanged form returns the existing order instead of
      // inserting a duplicate.
      setError(cause instanceof Error ? cause.message : "Не удалось создать ордер");
  ...
  return <section className={compactMode ? "min-h-0" : "border-t border-[var(--border-soft)] pt-4"}>
    ...
    <div className={`${compactMode ? "mt-2" : "mt-3"} grid grid-cols-2 gap-2`}><select value={kind} onChange={(event) => { invalidateCreateRequest(); setKind(event.target.value as Kind); }} className={compactMode ? "mxm-input !min-h-9 !text-[10px]" : "mxm-input"}><option value="limit_buy">Лимитная покупка</option><option value="limit_sell">Лимитная продажа</option><option value="take_profit">Фиксация прибыли</option><option value="stop_loss">Ограничение убытка</option></select><select value={durationDays} onChange={(event) => { invalidateCreateRequest(); setDurationDays(Number(event.target.value)); }} className={compactMode ? "mxm-input !min-h-9 !text-[10px]" : "mxm-input"}><option value={1}>1 день</option><option value={3}>3 дня</option><option value={7}>7 дней</option><option value={14}>14 дней</option><option value={30}>30 дней</option></select></div>
    <div className="mt-1.5 grid grid-cols-2 gap-2"><label className="block"><span className="mb-1 block text-[9px] text-[var(--muted)]">Триггер цены</span><input value={triggerPrice} onChange={(event) => { invalidateCreateRequest(); setTriggerPrice(event.target.value.replace(",", ".")); }} inputMode="decimal" placeholder={price(coin.currentPrice)} className={compactMode ? "mxm-input !min-h-9 !text-[10px]" : "mxm-input"} /></label><label className="block"><span className="mb-1 block text-[9px] text-[var(--muted)]">{isBuy ? "Сумма TON" : `Количество ${coin.symbol}`}</span><input value={inputAmount} onChange={(event) => { invalidateCreateRequest(); setInputAmount(event.target.value.replace(",", ".")); }} inputMode="decimal" placeholder="0" className={compactMode ? "mxm-input !min-h-9 !text-[10px]" : "mxm-input"} /></label></div>
    ...
    {isBuy && parsedInput != null && numericInput > 0 && numericInput < MIN_COIN_BUY_TON ? <p className="mt-2 text-[9px] text-[var(--negative)]">Минимальная сумма покупки — {MIN_COIN_BUY_TON} TON.</p> : null}
    {kind === "limit_buy" && parsedTrigger != null && numericTrigger > coin.currentPrice ? <p className="mt-2 text-[9px] text-[var(--negative)]">Лимитную покупку нужно ставить не выше текущей цены.</p> : null}
    {(kind === "limit_sell" || kind === "take_profit") && parsedTrigger != null && numericTrigger < coin.currentPrice ? <p className="mt-2 text-[9px] text-[var(--negative)]">Продажу и фиксацию прибыли нужно ставить не ниже текущей цены.</p> : null}
    {kind === "stop_loss" && parsedTrigger != null && numericTrigger > coin.currentPrice ? <p className="mt-2 text-[9px] text-[var(--negative)]">Stop Loss нужно ставить не выше текущей цены.</p> : null}
