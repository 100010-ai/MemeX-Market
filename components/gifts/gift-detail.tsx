      ...
      <div className="grid gap-3 md:grid-cols-[minmax(0,390px)_minmax(0,1fr)]">
        ...
        <section className="min-w-0 space-y-3">
          <div className="rounded-[18px] border border-[var(--border)] bg-[var(--panel)] p-3">
            ...
            <div className="mt-3 grid grid-cols-3 gap-2"><SmallMetric label="Объём предмета" value={money(data.itemStats.volume)} /><SmallMetric label="Сделки" value={String(data.itemStats.tradeCount)} /><SmallMetric label="Макс. продажа" value={data.itemStats.highSale == null ? "—" : money(data.itemStats.highSale)} /></div>
          </div>
...
function Activity({ trades }: { trades: GiftTrade[] }) { if (!trades.length) return <Empty text="Завершённых продаж пока нет" />; return <div className="divide-y divide-[var(--border-soft)]">{trades.map((trade) => <div key={trade.id} className="flex items-center justify-between gap-3 px-3 py-3"><div className="min-w-0"><p className="truncate text-xs"><span className="text-[var(--muted)]">{trade.sellerName || "—"}</span> → <span>{trade.buyerName}</span></p><p className="mt-0.5 text-[10px] text-[var(--muted)]">{ago(trade.createdAt)}</p></div><p className="flex items-center gap-1 text-xs font-medium"><Gem size={11} fill="currentColor" />{money(trade.price)}</p></div>)}</div>; }
