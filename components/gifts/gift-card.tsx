...
export function GiftCard({ gift, showOwner = false }: { gift: GiftAsset; showOwner?: boolean }) {
  ...
  return (
    <Link href={`/gifts/${gift.virtualGiftId}`} className="group min-w-0 overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--panel)] shadow-[inset_0_1px_0_rgba(255,255,255,.02)] transition active:scale-[.99]">
      ...
      <div className="p-2.5">
        ...
        <div className="mt-2.5 flex items-center gap-1.5">
          <span className="flex h-8 min-w-0 flex-1 items-center rounded-2xl bg-[var(--panel-2)] px-2.5 text-xs font-medium">
          ...
          {isListed ? <span className="grid h-8 w-8 shrink-0 place-items-center rounded-2xl bg-[var(--panel-2)] text-[var(--muted)] group-active:text-white"><ShoppingCart size={15} /></span> : null}
        </div>
      </div>
    </Link>
  );
