...
export function GiftCard({ gift, showOwner = false }: { gift: GiftAsset; showOwner?: boolean }) {
  ...
  const valueLabel = isListed ? "" : "est. ";

  return (
    <Link href={`/gifts/${gift.virtualGiftId}`} className="group min-w-0 overflow-hidden rounded-[7px] border border-[var(--border)] bg-[var(--panel)] transition active:scale-[.995]">
      ...
      <div className="p-2">
        ...
        <div className="mt-2 flex items-center gap-1.5">
          <span className="flex h-8 min-w-0 flex-1 items-center rounded-md bg-[var(--panel-2)] px-2 text-xs font-medium">
            {displayValue == null ? (
              <span className="truncate text-[var(--muted)]">Unpriced</span>
            ) : (
              <>
                <Gem size={12} className="mr-1 shrink-0 text-[#d9dde2]" fill="currentColor" />
                <span className="truncate">{valueLabel}{money(displayValue).replace("$", "")}</span>
              </>
            )}
          </span>
          {isListed ? <span className="grid h-8 w-8 shrink-0 place-items-center rounded-md bg-[var(--panel-2)] text-[var(--muted)] group-active:text-white"><ShoppingCart size={15} /></span> : null}
        </div>
      </div>
    </Link>
  );
