  ...
  return (
    <div className="mx-auto min-h-screen max-w-[1280px] lg:grid lg:grid-cols-[218px_1fr]">
      <aside className="sticky top-0 hidden h-screen border-r border-[var(--border-soft)] bg-[var(--bg)] p-3 lg:flex lg:flex-col">
        ...
        <Link href="/profile" className="mt-auto rounded-xl border border-[var(--border)] bg-[var(--panel)] p-3"><div className="flex items-center gap-2"><ProfileAvatar photoUrl={profile.photoUrl} /><div className="min-w-0"><p className="truncate text-xs font-medium">{profile.username ? `@${profile.username}` : profile.firstName}</p><p className="text-[10px] text-[var(--muted)]">Lv. {profile.level} · {profile.tier}</p></div></div><div className="mt-3 border-t border-[var(--border-soft)] pt-2"><div className="flex items-end justify-between gap-2"><div><p className="text-[10px] text-[var(--muted)]">Net worth</p><p className="mt-0.5 text-sm font-semibold">{money(profile.netWorth)}</p></div><span className="text-[9px] text-[var(--muted)]">{profile.xp} XP</span></div><div className="mt-2 h-1 overflow-hidden rounded-full bg-[var(--panel-2)]"><div className="h-full bg-[var(--accent)]" style={{ width: `${Math.round(profile.levelProgress * 100)}%` }} /></div></div></Link>
      </aside>
