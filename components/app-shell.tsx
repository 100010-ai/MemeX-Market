...
import { Boxes, Gamepad2, Gem, ListChecks, Menu, ReceiptText, Store, Trophy, UserRound, WalletCards } from "lucide-react";
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  if (loading) {
    return <div className="mx-auto min-h-screen max-w-md px-3 pt-3"><div className="mxm-skeleton h-12 rounded-lg" /><div className="mxm-skeleton mt-3 h-40 rounded-lg" /><div className="mt-3 grid grid-cols-2 gap-2"><div className="mxm-skeleton aspect-square rounded-lg" /><div className="mxm-skeleton aspect-square rounded-lg" /></div></div>;
  }
    ...
    <div className="mx-auto min-h-screen max-w-[1280px] lg:grid lg:grid-cols-[218px_1fr]">
      <aside className="sticky top-0 hidden h-screen border-r border-[var(--border-soft)] bg-[var(--bg)] p-3 lg:flex lg:flex-col">
        ...
        <Link href="/profile" className="mt-auto rounded-xl border border-[var(--border)] bg-[var(--panel)] p-3">
          <div className="flex items-center gap-2"><ProfileAvatar photoUrl={profile.photoUrl} /><div className="min-w-0"><p className="truncate text-xs font-medium">{profile.username ? `@${profile.username}` : profile.firstName}</p><p className="text-[10px] text-[var(--muted)]">{profile.tier}</p></div></div>
      ...
      <div className="min-w-0 pb-[70px] lg:pb-0">
        <header className="sticky top-0 z-40 border-b border-[var(--border-soft)] bg-[rgba(16,17,18,.97)] backdrop-blur-xl">
          <div className="flex h-[58px] items-center gap-2 px-3 md:px-4">
            ...
            <Link href="/vault" className="flex h-8 items-center gap-1.5 rounded-lg border border-[var(--border)] bg-[var(--panel)] px-2.5 text-xs font-medium lg:ml-auto">
              <Gem size={13} className="text-[var(--accent)]" fill="currentColor" />
              {money(profile.balance).replace("$", "")}
            </Link>

            <div className="ml-auto flex items-center gap-1.5 lg:ml-0">
              <Link href="/vault" aria-label="Wallet" className="header-action"><WalletCards size={16} /></Link>
              <Link href="/leaderboard" aria-label="Leaderboard" className="header-action"><Trophy size={16} /></Link>
              <Link href="/profile" aria-label="Menu" className="header-action"><Menu size={17} /></Link>
            </div>
          </div>
        </header>
      ...
      <nav className="safe-bottom fixed inset-x-0 bottom-0 z-50 grid grid-cols-5 border-t border-[var(--border-soft)] bg-[rgba(19,20,21,.98)] pt-1 backdrop-blur-xl lg:hidden">
        {nav.map((item) => {
          const active = pathname === item.href || pathname.startsWith(`${item.href}/`);
          const Icon = item.icon;
          return (
            <Link key={item.href} href={item.href} className={`relative flex min-w-0 flex-col items-center gap-0.5 px-1 pb-1 pt-1.5 text-[10px] ${active ? "text-white" : "text-[var(--muted)]"}`}>
              {active ? <span className="absolute -top-1 h-0.5 w-7 rounded-full bg-white" /> : null}
              <span className="grid h-7 w-9 place-items-center"><Icon size={18} strokeWidth={active ? 2.3 : 1.8} /></span>
