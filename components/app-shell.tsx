...
import { Boxes, Gamepad2, Gem, ListChecks, ReceiptText, Store, Trophy, UserRound } from "lucide-react";
  ...
  return (
    <div className="mx-auto min-h-[100dvh] max-w-[1280px] lg:grid lg:grid-cols-[220px_1fr]">
      ...
      <div className="min-w-0 pb-[calc(72px+env(safe-area-inset-bottom))] lg:pb-0">
        ...
        <main className="min-h-0 px-2.5 py-3 md:px-4 md:py-4">{children}</main>
      </div>

      <nav className="safe-bottom fixed inset-x-0 bottom-0 z-50 grid grid-cols-5 border-t border-[var(--border-soft)] bg-[rgba(7,9,11,.96)] px-1 pt-1 backdrop-blur-2xl lg:hidden">
        {nav.map((item) => {
          const active = pathname === item.href || pathname.startsWith(`${item.href}/`);
          const Icon = item.icon;
