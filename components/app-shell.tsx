...
export function AppShell({ children, modal }: { children: React.ReactNode; modal?: React.ReactNode }) {
  ...
  return (
    <div className="mx-auto min-h-[100dvh] max-w-[1280px] lg:grid lg:grid-cols-[204px_1fr]">
      <aside className="sticky top-0 hidden h-screen border-r border-[var(--border-soft)] bg-[rgba(5,6,7,.98)] p-2.5 lg:flex lg:flex-col">
        <Link href="/market" className="px-2 py-2.5 text-sm font-semibold tracking-[-.02em]">MXM</Link>
        <nav className="mt-1.5 space-y-1">
          {nav.map((item) => {
            const active = pathname === item.href || pathname.startsWith(`${item.href}/`);
            const Icon = item.icon;
            return <Link key={item.href} href={item.href} className={`flex items-center gap-3 rounded-[15px] px-2.5 py-2 text-[11px] transition-colors duration-150 ${active ? "bg-[var(--panel-2)] text-white" : "text-[var(--muted)] hover:bg-[var(--panel)] hover:text-white"}`}><Icon size={17} />{item.label}</Link>;
          })}
          <Link href="/leaderboard" className={`flex items-center gap-3 rounded-[15px] px-2.5 py-2 text-[11px] transition ${pathname.startsWith("/leaderboard") ? "bg-[var(--panel-2)] text-white" : "text-[var(--muted)] hover:bg-[var(--panel)] hover:text-white"}`}><Trophy size={17} />Рейтинг</Link>
        </nav>
        <Link href="/profile" className="mt-auto rounded-[16px] border border-[var(--border)] bg-[var(--panel)] p-2.5">
      ...
      <div className="min-w-0 pb-[calc(64px+env(safe-area-inset-bottom))] lg:pb-0">
        <header className="sticky top-0 z-40 border-b border-[var(--border-soft)] mxm-glassbar bg-[rgba(5,6,7,.96)]">
          <div className="flex h-[48px] items-center gap-2 px-2.5 md:px-3.5">
            <Link href="/profile" aria-label="Профиль" className="shrink-0 lg:hidden"><ProfileAvatar photoUrl={profile.photoUrl} /></Link>
            <div className="hidden lg:block"><p className="text-[11px] font-semibold">MemeX Market</p></div>
            <Link href="/vault" className="ml-auto flex h-8 items-center gap-1.5 border-b border-[var(--border)] px-1 text-[11px] font-medium lg:ml-auto" title={profile.reservedBalance > 0 ? `${money(profile.availableBalance)} доступно · ${money(profile.reservedBalance)} зарезервировано` : undefined}><Gem size={12} className="text-[var(--accent)]" fill="currentColor" />{money(profile.balance)}</Link>
          </div>
        </header>
        <main key={pathname} className="mxm-page-enter min-h-0 px-2 py-2.5 md:px-3.5 md:py-3">{children}</main>
      </div>

      <nav className="safe-bottom fixed inset-x-0 bottom-0 z-50 grid grid-cols-6 border-t border-[var(--border-soft)] mxm-glassbar bg-[rgba(7,9,11,.985)] px-0.5 pt-0.5 lg:hidden">
        {nav.map((item) => {
          const active = pathname === item.href || pathname.startsWith(`${item.href}/`);
          const Icon = item.icon;
          return <Link key={item.href} href={item.href} className={`relative flex min-w-0 flex-col items-center gap-0 px-0.5 pb-0.5 pt-0.5 text-[8px] transition-colors duration-150 ${active ? "text-white" : "text-[var(--muted)]"}`}><span className="grid h-7 w-9 place-items-center"><Icon size={16} strokeWidth={active ? 2.2 : 1.7} /></span><span className="max-w-full truncate">{item.label}</span></Link>;
        })}
      </nav>
