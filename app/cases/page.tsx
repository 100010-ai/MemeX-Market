...
export default function CasesPage() {
  ...
  return <div className="mx-auto max-w-5xl">
    <header className="mb-4 flex items-end justify-between gap-3 border-b border-[var(--border-soft)] pb-4"><div><p className="text-[10px] uppercase tracking-[.14em] text-[var(--muted-2)]">Коллекции</p><h1 className="mt-1 text-[20px] font-semibold tracking-[-.035em]">Кейсы и ограниченные серии</h1><p className="mt-1.5 text-[10px] leading-5 text-[var(--muted)]">Шансы раскрыты до открытия. Все награды виртуальные и не имеют денежной стоимости.</p></div><Link href="/store?category=cases" className="mxm-quick-link"><Star size={13} fill="currentColor" />Купить</Link></header>
    ...
    <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_330px]">
      <section className="mxm-card p-4">
        ...
        <div className={`mxm-case-stage mt-4 grid min-h-[260px] place-items-center rounded-[20px] border border-[var(--border)] bg-[var(--surface)] p-5 text-center ${opening ? "is-opening" : ""}`}>
          {reward ? <div className="mxm-case-reward"><Sparkles size={34} className="mx-auto text-[#f5c451]" /><p className="mt-3 text-[9px] uppercase tracking-[.16em] text-[var(--muted)]">{rarityLabel(reward.rarity)}</p><p className="mt-1 text-lg font-semibold">{reward.label}</p><p className="mt-2 text-[9px] text-[var(--muted)]">Награда добавлена в аккаунт</p></div> : <div><PackageOpen size={48} className="mx-auto text-[var(--accent)]" /><h2 className="mt-4 text-base font-semibold">{current?.title || "Загрузка…"}</h2><p className="mx-auto mt-2 max-w-sm text-[10px] leading-5 text-[var(--muted)]">{current?.description}</p>{current?.remaining != null ? <p className="mt-3 text-[9px] text-[#f3d789]">Осталось: {current.remaining.toLocaleString("ru-RU")}</p> : null}</div>}
        </div>
