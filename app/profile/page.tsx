  ...
  return (
    <div className="mx-auto max-w-2xl">
      <section className="rounded-[18px] border border-[var(--border)] bg-[var(--panel)] p-3.5">
        ...
        <div className="mt-2 rounded-[18px] bg-[var(--surface)] p-3"><div className="flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-2"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-[15px] bg-[var(--panel-2)] text-[var(--accent)]"><Sparkles size={14} /></span><div className="min-w-0"><p className="text-xs font-medium">Уровень {profile.level}</p><p className="truncate text-[9px] text-[var(--muted)]">{profile.xp} XP · {profile.xpForNextLevel} до следующего</p></div></div><span className="shrink-0 text-[9px] text-[var(--muted)]">Активность</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[var(--panel-2)]"><div className="h-full bg-[var(--accent)]" style={{ width: `${Math.round(profile.levelProgress * 100)}%` }} /></div></div>
