  ...
  return (
    <div className="mx-auto max-w-3xl">
      <header className="mb-4 border-b border-[var(--border-soft)] pb-4">
        <p className="text-[10px] uppercase tracking-[.14em] text-[var(--muted-2)]">Profile Studio</p>
        <h1 className="mt-1 text-[20px] font-semibold tracking-[-.035em]">Оформление профиля</h1>
        <p className="mt-1.5 text-[10px] leading-5 text-[var(--muted)]">Рамки профиля и коллекционные бейджи — только косметика. Они не меняют цену активов и торговые шансы.</p>
      </header>
      {notice ? <div className="mxm-alert mb-3">{notice}</div> : null}
      <section className="mxm-card mb-3 p-4">
        <div className="flex items-center gap-3">
          <div className="relative">
            <ProfileAvatar photoUrl={profile?.photoUrl || null} name={profile?.firstName || "MXM Trader"} equippedFrame={equippedFrame?.key || null} size="large" />
            {data?.wallet.premiumActive ? <Crown size={15} className="absolute -right-1 -top-1 text-[#f5c451]" fill="currentColor" /> : null}
          </div>
          <div className="min-w-0 flex-1">
            <p className="truncate text-[13px] font-semibold">{profile?.username ? `@${profile.username}` : profile?.firstName || "MXM Trader"}</p>
            <p className="mt-1 flex items-center gap-1 text-[9px] text-[var(--muted)]"><ShieldCheck size={10} />Trader {data?.wallet.vipTier || "Bronze"}</p>
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {data?.items.length ? data.items.map((item) => (
          <article key={item.key} className="mxm-card flex items-center gap-3 p-3">
            ...
            <div className="min-w-0 flex-1"><p className="truncate text-[11px] font-medium">{item.title}</p><p className="mt-1 text-[8px] uppercase text-[var(--muted)]">{item.type}</p></div>
          ...
          <div className="mxm-card col-span-full p-8 text-center"><Palette size={23} className="mx-auto text-[var(--muted)]" /><p className="mt-3 text-[11px]">Предметов оформления пока нет</p><Link href="/store?category=profile" className="mt-3 inline-block text-[10px] text-[var(--accent)]">Открыть MXM Store</Link></div>
        )}
      </div>
    </div>
  );
