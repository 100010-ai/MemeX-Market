...
export default function VaultPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <section className="mb-3 rounded-xl border border-[var(--border)] bg-[var(--panel)] p-3">
      ...
      <div className="mb-3 grid grid-cols-4 rounded-xl border border-[var(--border)] bg-[var(--panel)] p-1"><Tab label={`Gifts ${data.gifts.length}`} active={tab === "gifts"} onClick={() => setTab("gifts")} /><Tab label={`Coins ${data.holdings.length}`} active={tab === "coins"} onClick={() => setTab("coins")} /><Tab label={`Listed ${listed.length}`} active={tab === "listed"} onClick={() => setTab("listed")} /><Tab label="History" active={tab === "history"} onClick={() => setTab("history")} /></div>

      {tab === "gifts" || tab === "listed" ? (
        (tab === "listed" ? listed : data.gifts).length ? <div className="market-grid grid gap-2.5">{(tab === "listed" ? listed : data.gifts).map((gift) => <GiftCard key={gift.virtualGiftId} gift={gift} />)}</div> : <Empty title={tab === "listed" ? "No active listings" : "No virtual Gifts"} action={tab === "gifts" ? <PrimaryButton onClick={sync} disabled={syncing}>Sync Telegram Gifts</PrimaryButton> : undefined} />
      ) : tab === "coins" ? (
...
function Empty({ title, action }: { title: string; action?: React.ReactNode }) { return <div className="rounded-xl border border-[var(--border)] bg-[var(--panel)] p-10 text-center"><WalletCards className="mx-auto text-[var(--muted-2)]" /><p className="mt-3 text-sm font-medium">{title}</p>{action ? <div className="mt-4">{action}</div> : null}</div>; }
