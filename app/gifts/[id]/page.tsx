...
export default function GiftPage() {
  ...
  if (!data) return <div className="mx-auto max-w-5xl"><div className="mxm-skeleton h-[560px] rounded-2xl" />{error ? <p className="mt-3 text-xs text-[var(--negative)]">{error}</p> : null}</div>;
  const { gift } = data;
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mb-3 flex items-center justify-between gap-3">
        <Link href="/market" className="grid h-9 w-9 place-items-center rounded-2xl border border-[var(--border)] bg-[var(--panel-2)] text-[var(--muted)]"><ArrowLeft size={17} /></Link>
        <a href={`https://t.me/nft/${encodeURIComponent(gift.telegramName)}`} target="_blank" rel="noreferrer" className="flex h-9 items-center gap-1.5 rounded-2xl border border-[var(--border)] bg-[var(--panel-2)] px-3 text-[11px] text-[var(--muted)]">Открыть в Telegram <ExternalLink size={12} /></a>
      </div>

      <div className="grid gap-3 md:grid-cols-[minmax(0,390px)_minmax(0,1fr)]">
        <section className="min-w-0">
          <div className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--panel)]"><GiftMedia gift={gift} className="aspect-square w-full" /></div>
          <div className="mt-3 rounded-2xl border border-[var(--border)] bg-[var(--panel)] p-3">
            <div className="flex items-start justify-between gap-3"><div className="min-w-0"><h1 className="truncate text-lg font-semibold">{gift.baseName}</h1><p className="mt-0.5 text-xs text-[var(--muted)]">#{gift.number} · {gift.telegramName}</p></div>{gift.isFromBlockchain ? <span className="rounded-2xl bg-[var(--panel-2)] px-2 py-1 text-[9px] text-[var(--muted)]">Источник TON</span> : null}</div>
            <Link href={`/u/${gift.ownerId}`} className="mt-3 flex items-center gap-2 rounded-2xl bg-[var(--panel-2)] px-3 py-2.5"><UserRound size={14} className="text-[var(--muted)]" /><div className="min-w-0 flex-1"><p className="text-[10px] text-[var(--muted)]">Текущий владелец в MXM</p><p className="truncate text-xs font-medium">{gift.ownerName}</p></div></Link>
          </div>
        </section>
        ...
        <section className="min-w-0 space-y-3">
          ...
          {error ? <div className="rounded-2xl border border-[#5a3035] bg-[#25191b] px-3 py-2.5 text-xs text-[#ff9aa4]">{error}</div> : null}

          <div className="rounded-2xl border border-[var(--border)] bg-[var(--panel)] p-3">
            {gift.isBurned ? <div className="rounded-2xl border border-[#5a3035] bg-[#25191b] px-3 py-3 text-xs text-[#ff9aa4]">Telegram пометил этот подарок как сожжённый. В MXM для него отключены продажа, офферы и покупки.</div> : data.isOwner ? (
          ...
          <div className="grid grid-cols-3 rounded-2xl border border-[var(--border)] bg-[var(--panel)] p-1">
...
function BuyerTradePanel({ gift, availableBalance, reservedBalance, offerAmount, setOfferAmount, myOffer, busy, onBuy, onOffer, onCancelOffer }: { gift: GiftAsset; availableBalance: number; reservedBalance: number; offerAmount: string; setOfferAmount: (value: string) => void; myOffer?: DetailOffer; busy: string | null; onBuy: () => void; onOffer: (amount: number) => void; onCancelOffer?: () => void }) {
  ...
  return <><div className="mb-2 flex items-center justify-between text-[10px] text-[var(--muted)]"><span>Доступно {money(availableBalance)}</span>{reservedBalance > 0 ? <span>{money(reservedBalance)} в резерве</span> : null}</div>{gift.status === "listed" && gift.listingPrice != null ? <PrimaryButton className="flex w-full items-center justify-center gap-2 py-3" disabled={busy !== null || availableBalance + (myOffer?.amount || 0) < gift.listingPrice} onClick={onBuy}><ShoppingCart size={16} />{busy === "buy" ? "Покупка…" : <span className="flex items-center gap-1" >Купить <Gem size={13} fill="currentColor" />{money(gift.listingPrice).replace("$", "")}</span>}</PrimaryButton> : <div className="rounded-2xl bg-[var(--panel-2)] px-3 py-2.5 text-center text-xs text-[var(--muted)]">Не выставлен</div>}<div className="mt-2 flex gap-2"><input value={offerAmount} onChange={(event) => setOfferAmount(event.target.value)} inputMode="decimal" placeholder={myOffer ? `Текущий ${myOffer.amount}` : "Сумма оффера"} className="min-w-0 flex-1 rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2.5 text-sm outline-none" /><SecondaryButton disabled={busy !== null || !Number.isFinite(parsed) || parsed <= 0 || parsed > availableBalance + (myOffer?.amount || 0)} onClick={() => onOffer(parsed)}>{myOffer ? "Обновить" : "Предложить"}</SecondaryButton></div>{myOffer && onCancelOffer ? <button disabled={busy !== null} onClick={onCancelOffer} className="mt-2 w-full rounded-2xl bg-[var(--panel-2)] py-2 text-[11px] text-[var(--muted)]">Отменить мой оффер · {money(myOffer.amount)}</button> : null}</>;
}
...
function Metric({ label, value, tone }: { label: string; value: string; tone?: number }) { return <div className="rounded-2xl bg-[var(--panel-2)] p-2.5"><p className="text-[10px] text-[var(--muted)]">{label}</p><p className={`mt-1 truncate text-xs font-semibold ${tone == null ? "" : tone >= 0 ? "text-[var(--positive)]" : "text-[var(--negative)]"}`}>{value}</p></div>; }
...
function TabButton({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string }) { return <button onClick={onClick} className={`flex items-center justify-center gap-1.5 rounded-2xl py-2 text-[11px] ${active ? "bg-[var(--panel-3)] text-white" : "text-[var(--muted)]"}`}>{icon}{label}</button>; }
...
function Offers({ offers, isOwner, busy, onAction }: { offers: DetailOffer[]; isOwner: boolean; busy: string | null; onAction: (id: string, action: "accept" | "reject" | "cancel") => void }) {
  ...
  return <div className="divide-y divide-[var(--border-soft)]">{offers.map((offer) => <div key={offer.id} className="p-3"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="truncate text-xs font-medium">{offer.buyerName}{offer.isMine ? <span className="ml-1.5 text-[9px] text-[var(--accent)]">ВЫ</span> : null}</p><p className="mt-0.5 text-[10px] text-[var(--muted)]">{ago(offer.createdAt)}</p></div><p className="flex items-center gap-1 text-sm font-semibold"><Gem size={12} fill="currentColor" />{money(offer.amount).replace("$", "")}</p></div>{isOwner ? <div className="mt-2 grid grid-cols-2 gap-2"><button disabled={busy !== null} onClick={() => onAction(offer.id, "reject")} className="flex items-center justify-center gap-1 rounded-2xl bg-[var(--panel-2)] py-2 text-xs"><X size={13} />Отклонить</button><button disabled={busy !== null} onClick={() => onAction(offer.id, "accept")} className="flex items-center justify-center gap-1 rounded-2xl bg-[var(--accent)] py-2 text-xs font-semibold text-black"><Check size={13} />Принять</button></div> : offer.isMine ? <button disabled={busy !== null} onClick={() => onAction(offer.id, "cancel")} className="mt-2 w-full rounded-2xl bg-[var(--panel-2)] py-2 text-xs text-[var(--muted)]">Отменить</button> : null}</div>)}</div>;
}
