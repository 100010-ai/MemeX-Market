...
function object(value: unknown): JsonMap { return value && typeof value === "object" && !Array.isArray(value) ? value as JsonMap : {}; }
function text(value: unknown, max = 500) { return String(value ?? "").trim().slice(0, max); }
function int(value: unknown, min: number, max: number) { const n = Number(value); return Number.isInteger(n) && n >= min && n <= max ? n : null; }
  ...
  try {
    ...
    const [stars, cases, loot, seasons, rewards, activity] = await Promise.all([
      ...
      supabase.from("case_loot_definitions")
        .select("id,case_sku,reward_key,reward_kind,reward_label,amount,weight,rarity,metadata,active").order("case_sku").order("weight", { ascending: false }).limit(2000),
      supabase.from("seasons")
        .select("id,season_key,title,starts_at,ends_at,active,created_at,week_number,metadata").order("starts_at", { ascending: false }).limit(60),
      supabase.from("season_rewards")
        .select("season_id,level,track,required_xp,reward_kind,reward_label,amount,metadata").order("season_id").order("level").limit(3000),
    ...
    const starProfileIds = (stars.data || []).map((row) => String(row.profile_id));
    const riskIds = (activityData.washPairs || []).flatMap((row) => [String(row.a || ""), String(row.b || "")]);
    const recipientIds = (activityData.topRecipients || []).map((row) => String(row.profileId || ""));
    const profileIds = [...new Set([...starProfileIds, ...riskIds, ...recipientIds].filter(validUuidLike))];
    const profiles = profileIds.length
      ? await supabase.from("profiles").select("id,telegram_id,username,first_name,is_banned,hidden_from_leaderboard,balance,mxm_coins,stars_spent,vip_points,created_at").in("id", profileIds)
      : { data: [], error: null };
    if (profiles.error) throw profiles.error;
    const profileMap = new Map((profiles.data || []).map((row) => [String(row.id), row]));

    const summary = { paid: 0, refunded: 0, pending: 0, expired: 0, paidStars: 0, refundedStars: 0, pendingStars: 0 };
    for (const row of stars.data || []) {
      const amount = Number(row.stars || 0);
      if (row.status === "paid") { summary.paid++; summary.paidStars += amount; }
      else if (row.status === "refunded") { summary.refunded++; summary.refundedStars += amount; }
      else if (row.status === "pending") { summary.pending++; summary.pendingStars += amount; }
      else if (row.status === "expired") summary.expired++;
    }

    const lootByCase = new Map<string, JsonMap[]>();
    for (const row of loot.data || []) {
      const key = String(row.case_sku);
      const rows = lootByCase.get(key) || [];
      rows.push(row as unknown as JsonMap);
      lootByCase.set(key, rows);
    }
    const caseRows = (cases.data || []).map((row) => {
      const items = lootByCase.get(String(row.sku)) || [];
      const activeWeight = items.filter((item) => item.active !== false).reduce((sum, item) => sum + Number(item.weight || 0), 0);
      return {
        ...row,
        activeWeight,
        loot: items.map((item) => ({
    ...
    for (const row of rewards.data || []) {
      ...
      const rows = rewardsBySeason.get(key) || [];
      rows.push(row as unknown as JsonMap);
      rewardsBySeason.set(key, rows);
    }
    const seasonRows = (seasons.data || []).map((row) => {
      const items = rewardsBySeason.get(String(row.id)) || [];
      return { ...row, rewardCount: items.length, maxLevel: Math.max(0, ...items.map((item) => Number(item.level || 0))), rewards: items };
    });
    ...
    return NextResponse.json({
      stars: { summary, purchases: (stars.data || []).map((row) => ({ ...row, profile: profileMap.get(String(row.profile_id)) || null })) },
      cases: caseRows,
      seasons: seasonRows,
      economy: {
        daily: Array.isArray(activityData.daily) ? activityData.daily : [],
        topRecipients: (activityData.topRecipients || []).map((row) => ({ ...row, profile: profileMap.get(String(row.profileId || "")) || null })),
      },
      risks,
...
async function POSTHandler(request: Request) {
  ...
  if (!(await enforceRateLimit(request, "admin-ops-advanced", String(admin.id), 90, 60))) return NextResponse.json({ error: "Слишком много операций" }, { status: 429 });
  ...
  try {
    if (action === "profiles.bulk_moderate") {
      const ids = Array.isArray(body.profileIds) ? [...new Set(body.profileIds.map(String).filter(validUuidLike))].slice(0, MAX_BULK) : [];
      ...
      await audit(actor, "profiles.bulk_moderate", "profile_batch", undefined, { requested: ids.length, changed: result.data?.length || 0, isBanned: body.isBanned, hiddenFromLeaderboard: body.hiddenFromLeaderboard });
    ...
    if (action === "case.update") {
      ...
      const result = await supabase.from("case_definitions").update(patch).eq("sku", sku).select("sku,title,active,remaining_supply,rare_pity,epic_pity,legendary_pity").maybeSingle();
      if (result.error) throw result.error;
    ...
    if (action === "season.update") {
      ...
      for (const [input, column] of [["startsAt", "starts_at"], ["endsAt", "ends_at"]] as const) {
        if (body[input] !== undefined) {
          const date = new Date(String(body[input]));
          if (Number.isNaN(date.getTime())) return NextResponse.json({ error: `Некорректный ${input}` }, { status: 400 });
          patch[column] = date.toISOString();
        }
      }
      if (typeof body.active === "boolean") patch.active = body.active;
      if (!Object.keys(patch).length) return NextResponse.json({ error: "Нет изменений" }, { status: 400 });
      ...
      const result = await supabase.from("seasons").update(patch).eq("id", seasonId).select("id,title,active,week_number,starts_at,ends_at").maybeSingle();
      if (result.error) throw result.error;
      if (!result.data) return NextResponse.json({ error: "Сезон не найден" }, { status: 404 });
      await audit(actor, "season.update", "season", seasonId, patch);
      return NextResponse.json({ ok: true, season: result.data });
    }

    if (action === "stars.refund") {
      if (!(await enforceRateLimit(request, "admin-stars-refund-advanced", String(admin.id), 6, 3600))) return NextResponse.json({ error: "Лимит возвратов Stars временно исчерпан" }, { status: 429 });
      const purchaseId = text(body.purchaseId, 80);
      const reason = text(body.reason, 500);
      if (!validUuidLike(purchaseId) || reason.length < 5) return NextResponse.json({ error: "Укажите покупку и причину возврата" }, { status: 400 });
      ...
      if (purchase.data.status !== "paid" || chargeId.length < 4 || !Number.isSafeInteger(payerId) || payerId <= 0) return NextResponse.json({ error: "Refund доступен только для paid с Telegram charge ID" }, { status: 409 });
      let telegramRefunded = false;
      try {
        telegramRefunded = await telegramBotApi<boolean>("refundStarPayment", { user_id: payerId, telegram_payment_charge_id: chargeId });
      } catch (error) {
      ...
      if (telegramRefunded !== true) return NextResponse.json({ error: "Telegram не подтвердил возврат; локальное состояние не изменено" }, { status: 502 });
      const transition = await supabase.rpc("mark_star_purchase_refunded_v200", {
        p_purchase_id: purchaseId,
        p_charge_id: chargeId,
        p_reason: reason,
        p_metadata: { actor, source: "admin_advanced", fulfillmentReversal: "automatic_v074" },
      });
      ...
      await audit(actor, "stars.refund", "star_purchase", purchaseId, { reason, stars: Number(purchase.data.stars || 0), productSku: purchase.data.product_sku || null, reversal: reversal.data });
