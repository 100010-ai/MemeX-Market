...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!Number.isFinite(price) || price <= 0) return NextResponse.json({ error: "A positive listing price is required" }, { status: 400 });

  const supabase = getSupabaseAdmin();
  const { data: gift, error: giftError } = await supabase.from("virtual_gifts").select("owner_profile_id,status").eq("id", id).single();
  if (giftError || !gift) return NextResponse.json({ error: "Gift not found" }, { status: 404 });
  ...
  if (!owner.is_system) return NextResponse.json({ error: "This Gift is not system catalog inventory" }, { status: 400 });
