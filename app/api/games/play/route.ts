...
export async function POST(request: Request) {
  ...
  const game = ["coinflip","dice","wheel"].includes(body.game) ? String(body.game) : "";
  ...
  const { data, error } = await supabase.rpc("play_virtual_game", { p_profile_id: profile.id, p_game: game, p_bet: bet, p_choice: choice });
