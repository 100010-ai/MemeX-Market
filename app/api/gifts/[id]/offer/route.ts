...
async function POSTHandler(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (durationHours !== null && (!Number.isInteger(durationHours) || durationHours < 1 || durationHours > 168)) return NextResponse.json({ error: "Срок оффера должен быть от 1 до 168 часов" }, { status: 400 });
