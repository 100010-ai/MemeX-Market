...
export async function POST(request: Request) {
  try {
    ...
    if (!(await enforceRateLimit(request, "telegram-auth", "anonymous", 20, 600))) return NextResponse.json({ error: "Слишком много попыток входа. Попробуйте позже." }, { status: 429 });
