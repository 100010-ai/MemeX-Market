...
async function GETHandler(request: NextRequest) {
  ...
  const includeInventory = validUuidLike(requestedGiftId) || request.nextUrl.searchParams.get("includeInventory") === "1";
  const now = new Date().toISOString();
  try {
    const expireResult = await supabase
      .from("gift_trade_offers_v200")
    ...
    const [offersResult, ownGiftsResult] = await Promise.all([
      supabase
        .from("gift_trade_offers_v200")
        .select("id,sender_profile_id,recipient_profile_id,offered_gift_id,requested_gift_id,topup_amount,status,note,expires_at,resolved_at,created_at")
        .eq("status", "active")
        .or(`sender_profile_id.eq.${profile.id},recipient_profile_id.eq.${profile.id}`)
    ...
    for (const row of rows) {
      const offered = nonEmptyId(row.offered_gift_id);
      const requested = nonEmptyId(row.requested_gift_id);
      if (offered) giftIds.add(offered);
      if (requested) giftIds.add(requested);
      const sender = nonEmptyId(row.sender_profile_id);
      const recipient = nonEmptyId(row.recipient_profile_id);
      if (sender) profileIds.add(sender);
      if (recipient) profileIds.add(recipient);
    }
    if (validUuidLike(requestedGiftId)) giftIds.add(requestedGiftId);

    const [giftsResult, peopleResult] = await Promise.all([
      giftIds.size
        ? supabase.from("gift_market_overview").select(giftMarketSelect).in("virtual_gift_id", [...giftIds])
        : Promise.resolve({ data: [], error: null }),
      profileIds.size
        ? supabase.from("profiles").select("id,username,first_name,photo_url").in("id", [...profileIds])
        : Promise.resolve({ data: [], error: null }),
    ]);
    if (giftsResult.error || peopleResult.error) throw giftsResult.error || peopleResult.error;

    const gifts = new Map((giftsResult.data || []).map((row) => {
      const gift = mapGift(row);
      return [gift.virtualGiftId, gift] as const;
    }));
    const people = new Map((peopleResult.data || []).map((row) => [
      String(row.id),
      { name: row.username ? `@${row.username}` : row.first_name || "Пользователь", photoUrl: nullableText(row.photo_url, 2000) },
    ]));
    const offers = rows.flatMap((row) => {
      const id = nonEmptyId(row.id);
      const senderId = nonEmptyId(row.sender_profile_id);
      const recipientId = nonEmptyId(row.recipient_profile_id);
      const offeredId = nonEmptyId(row.offered_gift_id);
      const requestedId = nonEmptyId(row.requested_gift_id);
      if (!id || !senderId || !recipientId || !offeredId || !requestedId) return [];
      const offeredGift = gifts.get(offeredId);
      const requestedGift = gifts.get(requestedId);
      if (!offeredGift || !requestedGift) return [];
      return [{
        id,
        direction: senderId === String(profile.id) ? "outgoing" as const : "incoming" as const,
        senderId,
        senderName: people.get(senderId)?.name || "Пользователь",
        recipientId,
        recipientName: people.get(recipientId)?.name || "Пользователь",
        offeredGift,
        requestedGift,
        topupAmount: Math.max(0, finiteNumber(row.topup_amount)),
        status: text(row.status, "active", 24),
        note: nullableText(row.note, 240),
    ...
    return NextResponse.json({
      incoming: offers.filter((offer) => offer.direction === "incoming"),
      outgoing: offers.filter((offer) => offer.direction === "outgoing"),
    ...
    return apiFailure(error, "Не удалось загрузить Trade Center");
  }
}
...
async function POSTHandler(request: Request) {
  ...
  const { data, error } = await getSupabaseAdmin().rpc("create_gift_trade_offer_v200", {
    p_sender_id: profile.id,
    p_requested_gift_id: requestedGiftId,
    p_offered_gift_id: offeredGiftId,
