...
async function POSTHandler(request: Request) {
  ...
  const fixedPrice = body.fixedPrice == null || body.fixedPrice === "" ? null : parseEconomyAmount(body.fixedPrice);
  const floorOffsetPct = body.floorOffsetPct == null ? -3 : parseEconomyAmount(body.floorOffsetPct);
  const durationDays = body.durationDays == null ? 7 : parseEconomyAmount(body.durationDays);
  ...
  if (mode === "fixed" && (fixedPrice == null || fixedPrice < 0.01 || fixedPrice > 1_000_000_000)) return NextResponse.json({ error: "Некорректная единая цена" }, { status: 400 });
  if (floorOffsetPct == null || floorOffsetPct < -90 || floorOffsetPct > 1000) return NextResponse.json({ error: "Отклонение от минимальной цены должно быть от -90% до +1000%" }, { status: 400 });
