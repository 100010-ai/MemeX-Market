...
export async function GET(_request: Request, { params }: { params: Promise<{ name: string }> }) {
  ...
  if (!baseName) return NextResponse.json({ error: "Collection is required" }, { status: 400 });
  ...
  try {
    ...
    if (!collectionResult.data) return NextResponse.json({ error: "Collection not found" }, { status: 404 });
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Could not load collection" }, { status: 500 });
  }
}
