...
async function POSTHandler(request: NextRequest) {
  ...
  try {
    ...
    if (currentCart.error) throw currentCart.error;

    const rows = gifts.data || [];
    const ownerIds = [...new Set(rows.map((row) => String(row.owner_profile_id)).filter(Boolean))];
    const systemOwners = new Set<string>();
    if (liquidity.playerOnly && ownerIds.length) {
      const owners = await supabase.from("profiles").select("id,is_system").in("id", ownerIds);
      if (owners.error) throw owners.error;
      for (const owner of owners.data || []) if (owner.is_system) systemOwners.add(String(owner.id));
    }
    ...
    const valid = rows.filter((row) =>
      ...
      && (!row.listing_expires_at || new Date(row.listing_expires_at).getTime() > now)
      && (!liquidity.playerOnly || !systemOwners.has(String(row.owner_profile_id)))
    ).map((row) => String(row.id));

    if (!valid.length) return NextResponse.json({ error: "Выбранные лоты уже недоступны" }, { status: 409 });

    const existingIds = new Set((currentCart.data || []).map((row) => String(row.virtual_gift_id)));
    const capacity = Math.max(0, 20 - existingIds.size);
    const addIds = valid.filter((id) => !existingIds.has(id)).slice(0, capacity);
    if (!addIds.length) {
      if (capacity <= 0) return NextResponse.json({ error: "В корзине может быть максимум 20 подарков" }, { status: 409 });
      return NextResponse.json({ ok: true, added: [], count: existingIds.size });
    }

    const added = await supabase.from("market_cart_items").upsert(
      addIds.map((virtualGiftId) => ({ profile_id: profile.id, virtual_gift_id: virtualGiftId })),
      { onConflict: "profile_id,virtual_gift_id", ignoreDuplicates: true },
    );
