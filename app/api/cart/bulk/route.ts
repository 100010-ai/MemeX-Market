...
async function POSTHandler(request: NextRequest) {
  ...
  try {
    const [gifts, currentCart, liquidity] = await Promise.all([supabase.from("virtual_gifts").select("id,owner_profile_id,status,listing_price,listing_expires_at").in("id", ids), supabase.from("market_cart_items").select("virtual_gift_id").eq("profile_id", profile.id).limit(21), getGiftMarketLiquidityState()]);
    if (gifts.error) throw gifts.error; if (currentCart.error) throw currentCart.error;
    ...
    const unavailable = ids.filter((id) => !valid.includes(id)); const alreadyInCart = ids.filter((id) => existingIds.has(id)); const capacity = Math.max(0, 20 - existingIds.size); const addIds = valid.filter((id) => !existingIds.has(id)).slice(0, capacity);
    if (!addIds.length) { if (capacity <= 0) return NextResponse.json({ error: "В корзине может быть максимум 20 подарков" }, { status: 409 }); return NextResponse.json({ ok: true, added: [], unavailable, alreadyInCart, count: existingIds.size }); }
