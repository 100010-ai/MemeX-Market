...
async function GETHandler(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const [profileResult, coinsResult, giftsResult, reputationResult, achievementsResult, statsResult, verifiedEntitlementResult, badgeInventoryResult] = await Promise.all([
    ...
    const error = profileResult.error || coinsResult.error || giftsResult.error || reputationResult.error || achievementsResult.error || statsResult.error || verifiedEntitlementResult.error || badgeInventoryResult.error;
    ...
    return NextResponse.json({
      profile: {
        ...
        equippedProfileFrame: text(person.equipped_profile_frame, "", 120) || null,
        creatorVerified,
    ...
    return apiFailure(error, "Не удалось загрузить игрока");
  }
}
