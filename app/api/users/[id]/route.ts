...
function traderSnapshot(value: unknown) {
  ...
  return {
    ...
    giftSales: Math.max(0, Math.floor(finiteNumber(row.giftSales))),
    coinTradeCount: Math.max(0, Math.floor(finiteNumber(row.coinTradeCount))),
    createdCoinCount: Math.max(0, Math.floor(finiteNumber(row.createdCoinCount))),
    giftTradeVolume: Math.max(0, finiteNumber(row.giftTradeVolume)),
    coinTradeVolume: Math.max(0, finiteNumber(row.coinTradeVolume)),
    closedTrades: Math.max(0, Math.floor(finiteNumber(row.closedTrades))),
    winningTrades: Math.max(0, Math.floor(finiteNumber(row.winningTrades))),
    winRate: Math.max(0, Math.min(100, finiteNumber(row.winRate))),
    activeDays: Math.max(0, Math.floor(finiteNumber(row.activeDays))),
    collectorScore: Math.max(0, Math.min(100, finiteNumber(row.collectorScore))),
    collectorRank: Number.isFinite(rank) && rank > 0 ? Math.floor(rank) : null,
    giftCount: Math.max(0, Math.floor(finiteNumber(row.giftCount))),
    uniqueCollections: Math.max(0, Math.floor(finiteNumber(row.uniqueCollections))),
    rareGiftCount: Math.max(0, Math.floor(finiteNumber(row.rareGiftCount))),
    avgRarityScore: Math.max(0, Math.min(100, finiteNumber(row.avgRarityScore))),
...
async function GETHandler(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const [profileResult, coinsResult, giftsResult, reputationResult, achievementsResult, verifiedEntitlementResult, badgeInventoryResult, traderResult] = await Promise.all([
    ...
    const error = profileResult.error || coinsResult.error || giftsResult.error || reputationResult.error || achievementsResult.error || verifiedEntitlementResult.error || badgeInventoryResult.error || traderResult.error;
    ...
    const trader = traderSnapshot(traderResult.data);
    ...
    return NextResponse.json({ profile: {
      ...
      tradeCount: trader.tradeCount, giftCount: trader.giftCount, giftSales: trader.giftSales, giftTradeVolume: trader.giftTradeVolume, coinTradeCount: trader.coinTradeCount, coinTradeVolume: trader.coinTradeVolume, createdCoinCount: trader.createdCoinCount,
      trader,
