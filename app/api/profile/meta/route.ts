...
function traderSnapshot(value: unknown) {
  ...
  return {
    ...
    collectorRank: Number.isFinite(rawRank) && rawRank > 0 ? Math.floor(rawRank) : null,
...
async function GETHandler() {
  ...
  const [reputation, achievements, presentation, verifiedEntitlement, badgeInventory, trader] = await Promise.all([
    ...
    supabase
      ...
      .select("achievement_key,unlocked_at,achievements(title,description,icon,xp_reward,sort_order)", { count: "exact" })
  ...
  if (reputationStale) after(async () => {
    try {
      ...
      console.error("profile meta refresh", error);
    }
  });
  ...
  return NextResponse.json({
    reputation: {
      score: finiteNumber(rep.score, 50),
      tradeScore: finiteNumber(rep.trade_score),
      ageScore: finiteNumber(rep.age_score),
      activityScore: finiteNumber(rep.activity_score),
      trustScore: finiteNumber(rep.trust_score, 50),
      updatedAt: safeIsoDate(rep.updated_at),
    },
    trader: traderSnapshot(trader.data),
    achievements: (achievements.data || []).flatMap((raw) => {
      const value = mapAchievement(raw);
      return value ? [value] : [];
    }),
    achievementCount: Math.max(0, achievements.count || 0),
    appearance: {
