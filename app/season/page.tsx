...
export default function SeasonPage() {
  ...
  return <div className="mx-auto max-w-5xl">
    <header className="mb-4 border-b border-[var(--border-soft)] pb-4"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] uppercase tracking-[.14em] text-[var(--muted-2)]">30-day event</p><h1 className="mt-1 text-[20px] font-semibold tracking-[-.035em]">{data?.season.title || "Meme Season"}</h1><p className="mt-1.5 text-[10px] text-[var(--muted)]">Торгуй, собирай Gifts и создавай мемкоины, чтобы продвигаться по дорожке.</p></div><div className="shrink-0 text-right"><p className="flex items-center justify-end gap-1 text-[9px] text-[var(--muted)]"><Clock3 size={11} />Осталось</p><p className="mt-1 text-sm font-semibold">{data?.season.daysLeft ?? 30} дн.</p></div></div>
      <div className="mt-4 grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-full bg-[var(--accent)] text-[12px] font-black text-black">{data?.level || 1}</span><div><div className="flex justify-between text-[8px] text-[var(--muted)]"><span>{data?.xp || 0} XP</span><span>{next ? `${next.requiredXp} XP` : "MAX"}</span></div><div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-white/[.06]"><div className="h-full rounded-full bg-[var(--accent)]" style={{ width: `${progress}%` }} /></div></div>{data?.premium ? <span className="inline-flex items-center gap-1 text-[9px] text-[#f3d789]"><Crown size={10} />Premium Track открыт</span> : <Link href="/store?category=season" className="inline-flex items-center gap-1 text-[9px] text-[#f3d789]"><Star size={10} fill="currentColor" />Открыть Premium Track</Link>}</div>
    </header>
    {error ? <div className="mxm-alert mxm-alert-error mb-3">{error}</div> : null}
    <div className="grid grid-cols-[42px_minmax(0,1fr)_minmax(0,1fr)] gap-x-2 text-[9px]"><div /><div className="mb-2 flex items-center gap-1.5 text-[var(--muted)]"><Trophy size={12} />Free Track</div><div className="mb-2 flex items-center gap-1.5 text-[#f3d789]"><Crown size={12} />Premium Track</div>
...
function RewardCell({ reward, locked, claimed, busy, onClaim, premium = false }: { reward: Reward; locked: boolean; claimed: boolean; busy: boolean; onClaim: () => Promise<void>; premium?: boolean }) {
  return <div className={`flex min-h-[82px] items-center gap-2 border-t border-[var(--border-soft)] px-2 ${premium ? "bg-[#f5c451]/[.025]" : ""}`}><div className={`grid h-8 w-8 shrink-0 place-items-center rounded-[10px] ${premium ? "bg-[#f5c451]/10 text-[#f3d789]" : "bg-white/[.045] text-[var(--accent)]"}`}>{locked ? <LockKeyhole size={13} /> : claimed ? <Check size={14} /> : <Sparkles size={13} />}</div><div className="min-w-0 flex-1"><p className="truncate text-[9px] font-medium">{reward?.label || "Mystery reward"}</p>{!locked && !claimed ? <button type="button" disabled={busy} onClick={() => void onClaim()} className="mt-1 text-[8px] text-[var(--accent)] underline decoration-white/20 underline-offset-2">{busy ? "…" : "Забрать"}</button> : <p className="mt-1 text-[8px] text-[var(--muted-2)]">{claimed ? "Получено" : "Закрыто"}</p>}</div></div>;
}
