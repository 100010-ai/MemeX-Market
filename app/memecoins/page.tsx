...
function heatLabel(value: string) {
  return ({ quiet: "Тихо", moving: "Движение", trending: "Тренд", hot: "Hot", viral: "Viral" } as Record<string, string>)[value] || value;
}
...
export default function MemecoinsPage() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      const payload = await apiFetch<Payload>(`/api/memecoins/pulse?board=${board}&limit=36`, {
        cacheMs: 4_000,
      ...
      if (!controller.signal.aborted) setError(cause instanceof Error ? cause.message : "Не удалось загрузить мемкоины");
    } finally {
  ...
  return <div className="mx-auto max-w-6xl">
    <RealtimeRefresh channelName="mxm-memecoin-pulse-v200" tables={realtimeTables} onChange={realtimeReload} debounceMs={2_500} />
...
function TopMetric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-[16px] bg-[var(--panel-2)] p-3"><p className="text-[8px] uppercase tracking-[.08em] text-[var(--muted)]">{label}</p><p className="mt-1 truncate text-sm font-semibold">{value}</p></div>;
}
function Metric({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="rounded-[13px] bg-[var(--panel-2)] p-2"><p className="flex items-center gap-1 text-[7px] text-[var(--muted)]">{icon}{label}</p><p className="mt-1 truncate text-[10px] font-semibold">{value}</p></div>;
}
