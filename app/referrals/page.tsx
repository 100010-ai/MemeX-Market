...
export default function ReferralsPage() {
  ...
  return <div className="mx-auto max-w-3xl">
    <header className="mb-5 border-b border-[var(--border-soft)] pb-4"><p className="text-[10px] uppercase tracking-[.14em] text-[var(--muted-2)]">Партнёрка</p><h1 className="mt-1 text-[20px] font-semibold tracking-[-.035em]">Приглашай друзей</h1><p className="mt-1.5 max-w-xl text-[11px] leading-5 text-[var(--muted)]">Получай {data?.percent ?? 5}% бонусом от системных наград приглашённых: заданий, рекламы и покупок Stars. Торговый оборот не учитывается, чтобы рефералку нельзя было фармить круговыми сделками.</p></header>
    <div className="grid grid-cols-2 gap-x-6 border-b border-[var(--border-soft)] pb-4"><Metric label="Приглашено" value={String(data?.invitedCount ?? 0)} /><Metric label="Заработано" value={money(data?.totalEarned ?? 0)} /></div>
