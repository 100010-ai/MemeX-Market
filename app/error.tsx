  ...
  return (
    <div className="mx-auto flex min-h-[56vh] max-w-md items-center px-3">
      <div className="w-full rounded-[24px] border border-[var(--border)] bg-[var(--panel)] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,.02)]">
        ...
        <h1 className="mt-4 text-base font-semibold">Не удалось загрузить MXM</h1>
        <p className="mt-1.5 text-xs leading-5 text-[var(--muted)]">{error.message || "Страница не смогла завершить загрузку."}</p>
        {error.digest ? <p className="mt-2 font-mono text-[9px] text-[var(--muted-2)]">код {error.digest}</p> : null}
