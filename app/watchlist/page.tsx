...
type Payload = { coins: Coin[]; collections: GiftCollection[]; gifts: GiftAsset[]; alerts: Alert[]; watchlistMeta: { used: number; limit: number; premiumActive: boolean } };
...
export default function WatchlistPage() {
  ...
  async function remove(kind: "coin" | "gift" | "gift_collection", id: string) {
    ...
    try {
      ...
      await load();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Не удалось удалить из избранного"); }
    finally { setBusy(null); }
  }
  async function removeAlert(id: string) {
    if (busy) return;
    setBusy(`alert:${id}`); setError(null);
    try { await apiFetch("/api/alerts", { method: "POST", body: JSON.stringify({ action: "delete", id }) }); await load(); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "Не удалось удалить сигнал"); }
    finally { setBusy(null); }
  }
  ...
  return <div className="mx-auto max-w-5xl">
    <section className="mxm-summary-card p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] text-[var(--muted)]">Избранное</p><h1 className="mt-1 text-base font-semibold">Избранное и ценовые сигналы</h1><p className="mt-1 text-[10px] text-[var(--muted)]">Подарки, коллекции и мемкоины в одном месте.</p><p className="mt-2 text-[9px] text-[var(--muted)]">{data.watchlistMeta.used}/{data.watchlistMeta.limit} слотов{data.watchlistMeta.premiumActive ? " · Премиум ×2" : ""}</p></div><span className="grid h-10 w-10 place-items-center rounded-[16px] bg-[var(--panel-2)] text-[var(--accent)]"><Star size={18} fill="currentColor" /></span></div></section>
