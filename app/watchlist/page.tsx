...
export default function WatchlistPage() {
  ...
  return <div className="mx-auto max-w-5xl">
    <section className="mxm-summary-card p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] text-[var(--muted)]">Watchlist 2.0</p><h1 className="mt-1 text-base font-semibold">Избранное и ценовые сигналы</h1><p className="mt-1 text-[10px] text-[var(--muted)]">Gifts, коллекции и мемкоины в одном месте.</p><p className="mt-2 text-[9px] text-[var(--muted)]">{data.watchlistMeta.used}/{data.watchlistMeta.limit} слотов{data.watchlistMeta.premiumActive ? " · Premium ×2" : ""}</p></div><span className="grid h-10 w-10 place-items-center rounded-[16px] bg-[var(--panel-2)] text-[var(--accent)]"><Star size={18} fill="currentColor" /></span></div></section>
    ...
    {data.collections.length ? <section className="mt-4"><h2 className="mb-2 text-sm font-medium">Коллекции</h2><div className="grid gap-2 md:grid-cols-2">{data.collections.map((c) => <div key={c.baseName} className="mxm-card p-3"><div className="flex items-start justify-between gap-3"><Link href={`/collections/${encodeURIComponent(c.baseName)}`} className="min-w-0 flex-1"><p className="truncate text-xs font-semibold">{c.baseName}</p><div className="mt-2 grid grid-cols-3 gap-2 text-[10px]"><Metric label="Floor" value={c.floorPrice == null ? "—" : money(c.floorPrice)} /><Metric label="24ч" value={percent(c.change24h)} tone={c.change24h} /><Metric label="Лоты" value={String(c.listedCount)} /></div></Link><button disabled={Boolean(busy)} onClick={() => void remove("gift_collection", c.baseName)} className="p-2 text-[var(--muted)] disabled:opacity-40" aria-label="Удалить"><Trash2 size={14} /></button></div></div>)}</div></section> : null}
