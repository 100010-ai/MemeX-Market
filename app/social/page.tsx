...
export default function SocialPage() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      const payload = await apiFetch<Payload>(`/api/social/feed?mode=${mode}&limit=40`, {
        cacheMs: 1_200,
      ...
      if (!controller.signal.aborted) setError(cause instanceof Error ? cause.message : "Не удалось загрузить ленту");
    } finally {
  ...
  async function react(item: SocialItem, reaction: ReactionKey) {
    ...
    setData((current) => ({
      ...current,
      activity: current.activity.map((candidate) => {
    ...
    try {
      await apiFetch("/api/social/reactions", { method: "POST", body: JSON.stringify({ eventId: item.eventId, reaction }) });
    } catch (cause) {
      setData(previous);
  ...
  return <div className="mx-auto max-w-3xl">
    <RealtimeRefresh channelName="mxm-social-v200" tables={realtimeTables} onChange={realtimeReload} debounceMs={1_800} />
    <header className="mb-4 flex items-start justify-between gap-3">
      <div>
        <p className="text-[10px] uppercase tracking-[.13em] text-[var(--muted-2)]">MXM Community</p>
        <h1 className="mt-1 flex items-center gap-2 text-lg font-semibold"><UsersRound size={18} />Сообщество</h1>
        <p className="mt-1 text-[11px] leading-5 text-[var(--muted)]">Покупки, запуски, редкие события и Trade Offers в одном живом потоке.</p>
      </div>
      <button type="button" onClick={() => void load()} className="grid h-10 w-10 place-items-center rounded-[14px] bg-[var(--panel-2)] text-[var(--muted)]"><RefreshCw size={14} className={loading ? "animate-spin" : ""} /></button>
    </header>

    <div className="mb-3 grid grid-cols-2 gap-2">
      <button type="button" onClick={() => setMode("all")} className={`min-h-11 rounded-[15px] text-[11px] font-medium ${mode === "all" ? "bg-white text-black" : "bg-[var(--panel-2)] text-[var(--muted)]"}`}>Весь рынок</button>
      <button type="button" onClick={() => setMode("following")} className={`min-h-11 rounded-[15px] text-[11px] font-medium ${mode === "following" ? "bg-white text-black" : "bg-[var(--panel-2)] text-[var(--muted)]"}`}>Подписки · {data.followingCount}</button>
    </div>
...
function ReactionButton({ active, icon, value, onClick }: { active: boolean; icon: React.ReactNode; value: number; onClick: () => void }) {
  return <button type="button" onClick={onClick} className={`inline-flex min-h-8 items-center gap-1.5 rounded-[12px] px-2.5 text-[9px] ${active ? "bg-[rgba(139,164,255,.12)] text-[var(--accent)]" : "bg-[var(--panel-2)] text-[var(--muted)]"}`}>{icon}<span>{value || 0}</span></button>;
}
