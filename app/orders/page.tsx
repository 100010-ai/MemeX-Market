...
export default function OrdersPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      {tab === "listings" ? (
        data.listings.length ? <div className="space-y-1.5">{data.listings.map((gift) => <Link key={gift.virtualGiftId} href={`/gifts/${gift.virtualGiftId}`} className="grid grid-cols-[46px_minmax(0,1fr)_auto] items-center gap-2.5 rounded-2xl border border-[var(--border)] bg-[var(--panel)] p-2 active:bg-[var(--panel-2)]"><GiftMedia gift={gift} compact className="h-[46px] w-[46px] rounded-2xl" /><div className="min-w-0"><p className="truncate text-xs font-medium">{gift.baseName}</p><p className="mt-0.5 truncate text-[10px] text-[var(--muted)]">#{gift.number} · {gift.modelName}</p></div><div className="flex items-center gap-1 text-xs font-medium"><Gem size={11} fill="currentColor" />{gift.listingPrice == null ? "—" : money(gift.listingPrice)}</div></Link>)}</div> : <Empty text="Нет активных лотов" icon={<Tag />} />
      ) : current.length ? (
        <div className="space-y-1.5">{current.map((offer) => <div key={offer.id} className="rounded-2xl border border-[var(--border)] bg-[var(--panel)] p-2.5"><div className="grid grid-cols-[46px_minmax(0,1fr)_auto] items-center gap-2.5"><Link href={`/gifts/${offer.virtualGiftId}`}><GiftMedia gift={offer.gift} compact className="h-[46px] w-[46px] rounded-2xl" /></Link><div className="min-w-0"><Link href={`/gifts/${offer.virtualGiftId}`} className="block truncate text-xs font-medium">{offer.baseName} #{offer.number}</Link><p className="mt-0.5 truncate text-[10px] text-[var(--muted)]">{tab === "incoming" ? `от ${offer.buyerName}` : `владелец ${offer.ownerName}`} · {ago(offer.createdAt)}</p></div><p className="flex items-center gap-1 text-xs font-semibold"><Gem size={11} fill="currentColor" />{money(offer.amount)}</p></div><div className="mt-2 flex gap-2">{tab === "incoming" ? <><button disabled={busy !== null} onClick={() => act(offer.id, "reject")} className="flex-1 rounded-2xl bg-[var(--panel-2)] py-2 text-xs">Отклонить</button><button disabled={busy !== null} onClick={() => act(offer.id, "accept")} className="flex-1 rounded-[17px] bg-[var(--accent)] py-2 text-xs font-semibold text-black">Принять</button></> : <button disabled={busy !== null} onClick={() => act(offer.id, "cancel")} className="w-full rounded-2xl bg-[var(--panel-2)] py-2 text-xs">Отменить оффер</button>}</div></div>)}</div>
