...
export default function OrdersPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mb-3 grid grid-cols-3 gap-1 rounded-[20px] border border-[var(--border)] bg-[var(--panel)] p-1">
        <Tab label="Входящие" count={data.incoming.length} active={tab === "incoming"} onClick={() => setTab("incoming")} />
        <Tab label="Мои" count={data.outgoing.length} active={tab === "outgoing"} onClick={() => setTab("outgoing")} />
...
function Tab({ label, count, active, onClick }: { label: string; count: number; active: boolean; onClick: () => void }) { return <button onClick={onClick} className={`flex min-w-0 items-center justify-center gap-1 rounded-[16px] px-1.5 py-2 text-[10px] whitespace-nowrap ${active ? "bg-[var(--panel-3)] text-white" : "text-[var(--muted)]"}`}><span className="truncate">{label}</span><span className={`shrink-0 rounded-full px-1.5 py-0.5 text-[9px] ${active ? "bg-white/8" : "bg-[var(--surface)] text-[var(--muted-2)]"}`}>{count}</span></button>; }
