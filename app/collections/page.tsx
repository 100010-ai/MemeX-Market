...
export default function CollectionsPage() {
  ...
  async function claim(baseName: string) {
    ...
    try {
      ...
      haptic("heavy"); setNotice(`Complete Collection Bonus: +${Number(result.reward?.mxmCoins || 0).toLocaleString("ru-RU")} MXM Coins`); await load();
    ...
    <div className="mx-auto max-w-5xl">
      <header className="mb-4 border-b border-[var(--border-soft)] pb-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-[10px] uppercase tracking-[.14em] text-[var(--muted-2)]">Gift Collection</p>
            <h1 className="mt-1 text-[20px] font-semibold tracking-[-.035em]">Коллекционер · уровень {data?.level || 1}</h1>
            <p className="mt-1.5 text-[10px] text-[var(--muted)]">Уровень растёт от количества, редкости и уникальности Telegram Gifts.</p>
          </div>
          <div className="shrink-0 text-right">
            <p className="text-[9px] text-[var(--muted)]">Собрано</p>
            <p className="mt-1 text-sm font-semibold">{data?.giftCount || 0} Gifts</p>
          </div>
        </div>
        <div className="mt-3">
          <div className="flex justify-between text-[8px] text-[var(--muted)]"><span>{data?.totalPoints || 0} collection XP</span><span>{data?.nextLevel || 5} XP</span></div>
      ...
      <div className="mb-3 grid grid-cols-2 gap-2">
        <Metric icon={<Gift size={13} />} label="Gifts" value={String(data?.giftCount || 0)} />
      ...
      {data?.collections.length ? (
            ...
            <article key={item.baseName} className="mxm-card p-3.5">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-[9px] text-[var(--muted)]">{item.owned}/{item.target} · {item.rarityPoints} rarity XP · {item.holders} владельцев</p>
              ...
              <div className="mt-3 flex items-center justify-between border-t border-[var(--border-soft)] pt-3">
                <span className="text-[9px] text-[var(--muted)]">Floor {item.floorPrice == null ? "—" : money(item.floorPrice)}</span>
                {item.claimed ? (
                  <span className="inline-flex items-center gap-1 text-[9px] text-[var(--positive)]"><Check size={11} />Bonus получен</span>
                ) : item.complete ? (
                  ...
                  <span className="inline-flex items-center gap-1 text-[9px] text-[var(--muted-2)]"><LockKeyhole size={10} />Badge + MXM</span>
                )}
              </div>
            </article>
          ))}
        ...
        <div className="mxm-card grid min-h-56 place-items-center p-6 text-center">
          <div><Gem size={24} className="mx-auto text-[var(--muted)]" /><p className="mt-3 text-xs font-medium">Коллекция пуста</p><Link href="/market" className="mt-3 inline-block text-[10px] text-[var(--accent)]">Найти первый Gift</Link></div>
        </div>
      )}
    </div>
  );
