...
import { BarChart3, ChevronDown, Flame, Gift, ListFilter, Plus, Search, Sparkles } from "lucide-react";
...
export default function MarketPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mb-3 flex items-center rounded-lg border border-[var(--border)] bg-[var(--panel)] p-0.5">
        <button onClick={() => setTab("gifts")} className={`flex h-9 flex-1 items-center justify-center gap-2 rounded-md text-sm font-medium ${tab === "gifts" ? "bg-[var(--panel-3)] text-white" : "text-[var(--muted)]"}`}><Gift size={16} />Gifts</button>
        <button onClick={() => setTab("coins")} className={`flex h-9 flex-1 items-center justify-center gap-2 rounded-md text-sm font-medium ${tab === "coins" ? "bg-[var(--panel-3)] text-white" : "text-[var(--muted)]"}`}><BarChart3 size={16} />Coins</button>
      </div>
      ...
      <div className="mb-2.5 flex gap-2">
        <label className="flex h-10 min-w-0 flex-1 items-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--panel)] px-3">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={tab === "gifts" ? "Search NFT / number" : "Search coin / ticker"} className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-[var(--muted-2)]" />
        </label>
      ...
      {tab === "gifts" ? (
        <div className="mb-3 flex gap-2 overflow-x-auto pb-1 mxm-scrollbar-none">
          <FilterSelect value={collection} onChange={setCollection} label="NFT" options={data.collections.map((item) => item.baseName)} />
          ...
          <label className="relative flex h-9 shrink-0 items-center gap-1.5 rounded-lg border border-[var(--border)] bg-[var(--panel-2)] px-2.5 text-xs text-[#b8bbc1]"><ListFilter size={13} /><select aria-label="Sort" value={sort} onChange={(e) => setSort(e.target.value as typeof sort)} className="appearance-none bg-transparent pr-4 outline-none"><option value="price">Price</option><option value="number">Number</option><option value="rarity">Rarity</option></select><ChevronDown size={12} className="pointer-events-none absolute right-2" /></label>
        </div>
      ) : (
        <div className="mb-3 flex justify-end"><Link href="/create" className="flex h-9 items-center gap-1.5 rounded-lg bg-[var(--accent)] px-3 text-xs font-semibold text-black"><Plus size={14} />Create coin</Link></div>
      )}
      ...
      {tab === "gifts" ? (
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_280px]">
          <div>
            {loading ? <GridSkeleton /> : gifts.length ? <div className="market-grid grid gap-2.5">{gifts.map((gift) => <GiftCard key={gift.virtualGiftId} gift={gift} />)}</div> : <EmptyMarket icon={<Gift />} title="No Gift listings" text="The market only shows Telegram Gifts that players synced and listed. Open Vault to sync your collection." action={<Link href="/vault" className="inline-flex rounded-lg bg-[var(--panel-3)] px-4 py-2.5 text-sm font-medium">Open Vault</Link>} />}
          </div>
...
function FilterSelect({ value, onChange, label, options }: { value: string; onChange: (value: string) => void; label: string; options: string[] }) {
  return <label className="relative flex h-9 shrink-0 items-center rounded-lg border border-[var(--border)] bg-[var(--panel-2)] px-2.5 text-xs text-[#b8bbc1]"><select aria-label={label} value={value} onChange={(e) => onChange(e.target.value)} className="max-w-32 appearance-none bg-transparent pr-5 outline-none"><option value="all">{label}</option>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select><ChevronDown size={12} className="pointer-events-none absolute right-2" /></label>;
}

function CoinRow({ coin, index }: { coin: Coin; index: number }) {
  return (
    <Link href={`/coin/${coin.id}`} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-3 py-3 active:bg-[var(--panel-2)] md:grid-cols-[minmax(0,1.3fr)_0.8fr_0.8fr_0.8fr]">
...
function GridSkeleton() { return <div className="market-grid grid gap-2.5">{Array.from({ length: 8 }, (_, i) => <div key={i} className="overflow-hidden rounded-[9px] border border-[var(--border)] bg-[var(--panel)]"><div className="mxm-skeleton aspect-square" /><div className="p-2"><div className="mxm-skeleton h-4 rounded" /><div className="mxm-skeleton mt-2 h-8 rounded" /></div></div>)}</div>; }
