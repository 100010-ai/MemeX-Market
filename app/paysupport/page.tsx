...
export default function PaymentSupportPage() {
  ...
  return <main className="mx-auto min-h-[100dvh] max-w-2xl px-4 py-8 text-[11px] leading-5 text-[var(--muted)]">
    ...
    <p className="mt-3">Если Telegram уже показал статус «Оплачено», но товар ещё не появился, подождите несколько секунд и заново откройте магазин MXM. Повторное уведомление Telegram безопасно: одна покупка исполняется только один раз.</p>
    <div className="mt-6 flex gap-4"><Link href="/store" className="text-white underline decoration-white/30 underline-offset-4">магазин MXM</Link><Link href="/terms" className="text-white underline decoration-white/30 underline-offset-4">Условия</Link></div>
  </main>;
}
