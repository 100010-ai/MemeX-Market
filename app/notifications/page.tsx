...
type Preferences = { gift_sold: boolean; gift_offer: boolean; offer_resolved: boolean; price_alert: boolean; coin_move: boolean; sponsored_task: boolean; referral_reward: boolean; promo: boolean; telegram_push: boolean };
...
export default function NotificationsPage() {
  ...
  return <div className="mx-auto max-w-3xl">
    ...
    {settings ? <section className="mxm-card mt-3 p-3"><p className="mb-2 text-xs font-medium">Какие события показывать</p><div className="space-y-1">{([['gift_sold','Продажи Gifts'],['gift_offer','Новые офферы'],['offer_resolved','Решение по офферу'],['price_alert','Ценовые алерты'],['coin_move','Движение мемкоинов'],['sponsored_task','Рекламные задания'],['referral_reward','Доход от рефералов'],['promo','Промокоды'],['telegram_push','Push через Telegram']] as [keyof Preferences,string][]).map(([key,label]) => <button key={key} onClick={() => void toggle(key)} className="flex w-full items-center justify-between rounded-[14px] px-2.5 py-2 text-left text-[11px] hover:bg-[var(--panel-2)]"><span>{label}</span><span className={`h-5 w-9 rounded-full p-0.5 transition ${data.preferences[key] ? 'bg-[var(--accent)]' : 'bg-[var(--panel-3)]'}`}><span className={`block h-4 w-4 rounded-full bg-white transition ${data.preferences[key] ? 'translate-x-4' : ''}`} /></span></button>)}</div></section> : null}
...
function iconFor(kind: string) { if (kind.includes('gift') || kind.includes('offer')) return <Gift size={13} />; if (kind.includes('referral')) return <UsersRound size={13} />; if (kind.includes('coin') || kind.includes('price')) return <TrendingUp size={13} />; if (kind.includes('promo') || kind.includes('sponsored')) return <Megaphone size={13} />; return <Bell size={13} />; }
