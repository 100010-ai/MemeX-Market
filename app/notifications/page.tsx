...
export default function NotificationsPage() {
  ...
  async function read(id: string) {
    ...
    try { await apiFetch("/api/notifications", { method: "POST", body: JSON.stringify({ action: "read", id }) }); }
  ...
  async function readAll() {
    ...
    try { await apiFetch("/api/notifications", { method: "POST", body: JSON.stringify({ action: "read_all" }) }); }
  ...
  async function toggle(key: keyof Preferences) {
    ...
    try { await apiFetch("/api/notifications", { method: "POST", body: JSON.stringify({ action: "preferences", ...next }) }); }
    catch (cause) { setData((current) => current ? { ...current, preferences: previous } : current); setError(cause instanceof Error ? cause.message : "Не удалось сохранить настройки"); }
  }
