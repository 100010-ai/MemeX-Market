  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        if (!cancelled) setQuoteError(cause instanceof Error ? cause.message : "Could not quote trade");
      }
    }, 220);
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        <button onClick={toggleWatch} disabled={watchBusy} aria-label={data.watched ? "Remove coin from watchlist" : "Add coin to watchlist"} className={`grid h-9 w-9 place-items-center rounded-lg border ${data.watched ? "border-[var(--accent)] bg-[rgba(255,212,0,.08)] text-[var(--accent)]" : "border-[var(--border)] bg-[var(--panel)] text-[var(--muted)]"}`}><Star size={16} fill={data.watched ? "currentColor" : "none"} /></button>
      </div>
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_340px]">
        <div className="min-w-0 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6"><Stat label="Market cap" value={money(coin.marketCap)} /><Stat label="24h volume" value={money(coin.volume24h)} /><Stat label="Liquidity" value={money(coin.liquidity)} /><Stat label="ATH" value={price(coin.athPrice)} /><Stat label="Holders" value={String(coin.holderCount)} /><Stat label="Trades" value={String(coin.tradeCount24h)} /></div>

          <section className="rounded-xl border border-[var(--border)] bg-[var(--panel)] p-3">
            <div className="mb-2 flex items-center justify-between gap-3"><div><p className="text-xs font-medium">24h order flow</p><p className="mt-0.5 text-[10px] text-[var(--muted)]">Completed MXM trades only</p></div><span className="text-[10px] text-[var(--muted)]">{flow > 0 ? `${buyShare.toFixed(0)}% buys` : "No volume"}</span></div>
            ...
            <div className="mt-2 flex justify-between text-[10px]"><span className="text-[var(--positive)]">Buy {money(coin.buyVolume24h)}</span><span className="text-[var(--negative)]">Sell {money(coin.sellVolume24h)}</span></div>
          </section>
        ...
        <aside className="space-y-3">
          <section className="rounded-xl border border-[var(--border)] bg-[var(--panel)] p-3 lg:sticky lg:top-[68px]">
            <div className="grid grid-cols-2 rounded-lg bg-[var(--surface)] p-1"><button onClick={() => { setSide("buy"); setAmount(""); setQuote(null); }} className={`rounded-md py-2 text-xs font-medium ${side === "buy" ? "bg-[var(--positive)] text-black" : "text-[var(--muted)]"}`}>BUY</button><button onClick={() => { setSide("sell"); setAmount(""); setQuote(null); }} className={`rounded-md py-2 text-xs font-medium ${side === "sell" ? "bg-[var(--negative)] text-white" : "text-[var(--muted)]"}`}>SELL</button></div>
            ...
            {side === "buy" && data.reservedBalance > 0 ? <p className="mt-1 text-right text-[9px] text-[var(--muted-2)]">{money(data.reservedBalance)} reserved by Gift offers</p> : null}
            ...
            <div className="mt-2 grid grid-cols-4 gap-1">{[0.1, 0.25, 0.5, 1].map((fraction) => <button key={fraction} onClick={() => setAmount(String(Number((max * fraction).toFixed(8))))} className="rounded-md bg-[var(--panel-2)] py-2 text-[10px] text-[var(--muted)] hover:text-white">{fraction === 1 ? "MAX" : `${fraction * 100}%`}</button>)}</div>

            {quote ? <div className="mt-3 space-y-2 rounded-lg border border-[var(--border-soft)] bg-[var(--surface)] p-2.5"><QuoteRow label="You receive" value={side === "buy" ? `${compact(quote.outputAmount)} ${coin.symbol}` : money(quote.outputAmount)} strong /><QuoteRow label="Execution price" value={price(quote.executionPrice)} /><QuoteRow label="Fee · 0.5%" value={money(quote.feeAmount)} /><QuoteRow label="Price impact" value={`${quote.priceImpact.toFixed(2)}%`} warning={quote.priceImpact >= 10} /><QuoteRow label="After trade" value={price(quote.projectedPrice)} /></div> : null}
            {quoteError ? <p className="mt-2 text-[10px] text-[var(--negative)]">{quoteError}</p> : null}
            {Number.isFinite(numericAmount) && numericAmount > max ? <p className="mt-2 text-[10px] text-[var(--negative)]">Amount exceeds available balance.</p> : null}
            {error ? <div className="mt-3 rounded-lg bg-[#25191b] px-3 py-2 text-xs text-[#ff9aa4]">{error}</div> : null}
            <PrimaryButton onClick={trade} disabled={busy || !quote || !Number.isFinite(numericAmount) || numericAmount <= 0 || numericAmount > max} className={`mt-3 w-full py-3 ${side === "sell" ? "!bg-[var(--negative)] !text-white" : "!bg-[var(--positive)]"}`}>{busy ? "Processing…" : `${side === "buy" ? "Buy" : "Sell"} $${coin.symbol}`}</PrimaryButton>
