  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        <button onClick={toggleWatch} disabled={watchBusy} aria-label={data.watched ? "Убрать коин из избранного" : "Добавить коин в избранное"} className={`grid h-9 w-9 place-items-center rounded-2xl border ${data.watched ? "border-[var(--accent)] bg-[rgba(255,212,0,.08)] text-[var(--accent)]" : "border-[var(--border)] bg-[var(--panel)] text-[var(--muted)]"}`}><Star size={16} fill={data.watched ? "currentColor" : "none"} /></button>
      </div>
