    ...
    <div className="mx-auto max-w-xl">
      ...
      <section className="rounded-2xl border border-[var(--border)] bg-[var(--panel)] p-3 md:p-4">
        <div className="grid gap-4">
          <Field label="Название"><input value={name} onChange={(e) => setName(e.target.value)} maxLength={32} placeholder="Грустный Кот" className="mt-2 w-full rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-3 outline-none focus:border-[#565a61]" /></Field>
          <Field label="Тикер"><div className="flex rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-3"><span className="py-3 text-[var(--muted)]">$</span><input value={symbol} onChange={(e) => setSymbol(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0,8))} placeholder="CAT" className="min-w-0 flex-1 bg-transparent px-1 py-3 outline-none" /></div></Field>
          <Field label="Описание"><textarea value={description} onChange={(e) => setDescription(e.target.value)} maxLength={180} rows={4} placeholder="Кот, переживший каждую красную свечу." className="mt-2 w-full resize-none rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-3 outline-none focus:border-[#565a61]" /><span className="mt-1 block text-right text-[10px] text-[var(--muted)]">{description.length}/180</span></Field>
        </div>
