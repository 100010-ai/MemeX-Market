...
export default function CreatePage() {
  ...
  return <div className="mx-auto max-w-xl"><SectionTitle title="Launch coin" subtitle="Create a player-traded virtual meme coin." /><section className="rounded-xl border border-[var(--border)] bg-[var(--panel)] p-3 md:p-4"><div className="grid gap-4"><Field label="Name"><input value={name} onChange={(e) => setName(e.target.value)} maxLength={32} placeholder="Keyboard Cat" className="mt-2 w-full rounded-lg border border-[var(--border)] bg-[var(--surface)] p-3 outline-none focus:border-[#565a61]" /></Field><Field label="Ticker"><div className="flex rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3"><span className="py-3 text-[var(--muted)]">$</span><input value={symbol} onChange={(e) => setSymbol(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0,8))} placeholder="KCAT" className="min-w-0 flex-1 bg-transparent px-1 py-3 outline-none" /></div></Field><Field label="Description"><textarea value={description} onChange={(e) => setDescription(e.target.value)} maxLength={180} rows={4} placeholder="The cat that survived every red candle." className="mt-2 w-full resize-none rounded-lg border border-[var(--border)] bg-[var(--surface)] p-3 outline-none focus:border-[#565a61]" /><span className="mt-1 block text-right text-[10px] text-[var(--muted)]">{description.length}/180</span></Field></div><div className="mt-4 rounded-lg bg-[var(--panel-2)] p-3 text-xs"><Row name="Launch fee" value="$50 virtual" /><Row name="Starting market cap" value="$100" /><Row name="Supply" value="1,000,000,000" /><Row name="Curve" value="Constant-product AMM" /></div>{error ? <p className="mt-3 text-xs text-[var(--negative)]">{error}</p> : null}<PrimaryButton onClick={create} disabled={busy || name.trim().length < 2 || symbol.length < 2} className="mt-4 flex w-full items-center justify-center gap-2 py-3"><Rocket size={17} />{busy ? "Launching…" : "Launch virtual coin"}</PrimaryButton></section></div>;
}
